...
public class Sprite extends DrawingArea {
	...
	public static Sprite copy(Sprite image) {
		Sprite copy = new Sprite();
		copy.myPixels = image.myPixels.clone();
		copy.myWidth = image.myWidth;
		copy.myHeight = image.myHeight;
		copy.drawOffsetX = image.drawOffsetX;
		copy.drawOffsetY = image.drawOffsetY;
		copy.maxWidth = image.maxWidth;
		copy.maxHeight = image.maxHeight;
		return copy;
	}
	
	/*	private static List<String> ims = new ArrayList<String>();
	static int  c = 0;*/
	public Sprite(String img) {
		String location = new StringBuilder().append(Signlink.getCacheDirectory()).toString();
/*if(ims.contains(img)) {
			c++;
		}else
			ims.add(img);*/
		try {
			...
			BufferedImage image = ImageIO.read(new File(location+img+".png"));//Toolkit.getDefaultToolkit().getImage(new StringBuilder().append(location).append(img).append(".png").toString());
			...
			image.getRGB(0, 0, myWidth, myHeight, myPixels, 0, myWidth);
			image = null;
	...
	public static BufferedImage cropImage(BufferedImage src, int cropFromTop, int cropFromBottom) {
		BufferedImage dest = src.getSubimage(0, cropFromTop, src.getWidth(), src.getHeight()-cropFromBottom-cropFromTop);
		return dest;
	}
	...
	public void setPixels(int x, int y, int color) {
		this.myPixels[x + y * this.myWidth] = color;
	}
