...
public class GrandExchange {
	...
	public void itemSearch(String n) {
		...
		label0: for (int id = 0; id < ItemDefinition.totalItems; id++) {
			...
			if(item == null)
				continue;
			if (item.note != -1 || item.name == null || item.name == "Picture"
					|| item.noteTemplate == 18786 || item.name == "Null" || item.name.toLowerCase().contains("coins") || item.value <= 0 || item.id == 4178 || item.id == 14661/*|| item.untradeable*/)
				continue;
