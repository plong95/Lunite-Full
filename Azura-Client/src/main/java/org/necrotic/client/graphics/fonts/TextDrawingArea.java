...
public final class TextDrawingArea extends DrawingArea {
    ...
    public void drawText1(int i, String s, int k, int l) {
    ...
    public void drawRegularText(boolean flag1, int i, int j, String s, int k) {
        ...
        for (int i1 = 0; i1 < s.length(); i1++) {
            ...
            if (character == 60) {
                ...
                if (character == 62 && startIndex != -1) {
                    String effectString = s.substring(startIndex + 1, i1);
                    startIndex = -1;

                    if (effectString.startsWith("col=")) {
                        String color = effectString.substring(4);
                        textColor = color.length() < 6 ? Color.decode(color).getRGB() : Integer.parseInt(color, 16);
                    }                     else if (effectString.equals("/col")) {
                        textColor = j;
                    }
                    continue;
                }
            ...
            if (startIndex == -1) {
                if (s.charAt(i1) == '@' && i1 + 4 < s.length() && s.charAt(i1 + 4) == '@') {
                    ...
                    if (c != ' ') {
                        ...
                        method392(aByteArrayArray1491[c], i + anIntArray1494[c], k + anIntArray1495[c], anIntArray1492[c], anIntArray1493[c], textColor);
                    }
                    i += rsb[c];
                }
            }
