...
public final class OnDemandFetcher extends OnDemandFetcherParent implements Runnable {
	...
	public void get(int type, int id) {
	...
	private void handleFailed() {
		...
		while (uncompletedCount < 10) {
			...
			try {
				...
				System.out.println("onDemandData_1.getDataType(): " + onDemandData_1.getDataType());
				System.out.println("onDemandData_1.getId(): " + onDemandData_1.getId());
				e.printStackTrace();
			}
		}
	}
	...
	public void start(Archive streamLoader, Client client1) {
		...
		mapIndices2 = new int[mapCount+ extraMaps];
		mapIndices3 = new int[mapCount+ extraMaps];
		int[] dntUse = new int[] {5181, 5182, 5183, 5184, 5180, 5179, 5175, 5176, 4014, 3997, 5314, 5315, 5172};
		for(int i2 = 0; i2 < mapCount; i2++)
		{
			regionIds[i2] = stream2.getUnsignedShort();
		...
		abyte2 = FileOperations.readFile(Signlink.getCacheDirectory() + "osrs/map_index");
		ByteBuffer osrsMapStream = new ByteBuffer(abyte2);
		...
		for (Integer region : OldschoolMaps.OLDSCHOOL_REGION_IDS) {
			...
			if (getOldschoolLandscapeMapId(region) != -1 && getOldschoolObjectMapId(region) != -1
					&& getIndicesIndex(region) != -1) {
				int here = getIndicesIndex(region);
				if (here != -1) {
					regionIds[here] = -1;
					mapIndices2[here] = -1;
					mapIndices3[here] = -1;
			...
			mapIndices2[currentCount] = getOldschoolObjectMapId(region);
			mapIndices3[currentCount++] = getOldschoolLandscapeMapId(region);
			}catch (Exception e){
				e.printStackTrace();
			}
		}
	...
	private void dumpMapIndex(int j1) throws IOException {
		try {
			...
			for (int id = 0; id < j1; id++) {
				try {
					//
					writer.append("mapIndices1[").append(String.valueOf(id)).append("] = ").append(String.valueOf(regionIds[id]));
					writer.newLine();
					writer.append("mapIndices2[").append(String.valueOf(id)).append("] = ").append(String.valueOf(mapIndices2[id]));
					writer.newLine();
					writer.append("mapIndices3[").append(String.valueOf(id)).append("] = ").append(String.valueOf(mapIndices3[id]));
					writer.newLine();
					writer.newLine();
					//
					writer.flush();
