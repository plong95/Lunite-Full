...
import org.necrotic.client.FloorDefinitionOSRS;
...
public final class ObjectManager {
	...
	public static void method173(ByteBuffer stream, OnDemandFetcher class42_sub1, DataType dataType) {
		label0: {
		int i = -1;
	...
	public static void method188(WorldController worldController, int i, int j, int k, int l, CollisionData class11, int ai[][][], int i1, int j1, int k1) {
		if (l == 4) {
			l = 0;
		}
		int l1 = ai[l][i1][j];
	...
	public final void createRegionScene(CollisionData aclass11[], WorldController worldController) {
		for (int j = 0; j < 4; j++) {
			for (int k = 0; k < 104; k++) {
			...
			if (l6 >= 1 && l6 < anInt146 - 1) {
				...
				for (int k17 = -5; k17 < anInt147 + 5; k17++) {
					...
					if (k17 >= 1 && k17 < anInt147 - 1 && (!(lowDetail || Configuration.TOGGLE_ROOF_OFF) || (aByteArrayArrayArray149[0][l6][k17] & 2) != 0 || (aByteArrayArrayArray149[l][l6][k17] & 0x10) == 0 && method182(k17, l, l6) == anInt131)) {
						...
						int overlayTile = aByteArrayArrayArray130[l][l6][k17] & 0xff;
						DataType overlayType = overlayDataType[l][l6][k17];
						if (l18 > 0 || overlayTile > 0) {
							...
							if (l > 0) {
								...
								if (overlayType == DataType.OLDSCHOOL) {
									...
									if (overlayTile > OverLayFlo317.overLayFlo317s.length) {
										overlayTile = OverLayFlo317.overLayFlo317s.length - 1;
									}
									if (overlayTile > 0 && !OverLayFlo317.overLayFlo317s[overlayTile - 1].aBoolean393) {
										flag = false;
									}
								}
							...
							if (overlayTile == 0) {
								...
								OverLayFlo317 flo_2 = OverLayFlo317.overLayFlo317s[overlayTile - 1];
								...
								if (overlayType == DataType.OLDSCHOOL) {
									...
									if (floorTexture >= 0) {
										...
										mapTexture = Rasterizer.anIntArray1482[method185(overlay_flo.hsl16, 96)];
										j23 = -2;
										floorTexture = -1;
									} else {
									...
									if (mapTexture == 0x000000 && overlay_flo.anotherRgb != -1) {
										...
										mapTexture = Rasterizer.anIntArray1482[method185(temporaryMinimapColor, 96)];
	...
	private void addObjectToRenderer(int regionId, int y, WorldController worldController, CollisionData class11, int type, int z, int x, int objectId, int face) {
		boolean osrs = OldschoolMaps.isOldschoolRegion(regionId);


		if (!osrs &&  (lowDetail || Configuration.TOGGLE_ROOF_OFF) && (aByteArrayArrayArray149[0][x][y] & 2) == 0) {
		...
		if (type == 22) {
			if (!osrs && lowDetail && !class46.hasActions && !class46.aBoolean736) {
				return;
			}
			Object obj;
			if (class46.animationID == -1 && class46.configObjectIDs == null) {
	...
	public final void loadMapChunk(int regionId, int i, int j, CollisionData aclass11[], int l, int i1, byte abyte0[], int j1, int k1, int l1) {
			...
			for (int i3 = 0; i3 < 64; i3++) {
				for (int j3 = 0; j3 < 64; j3++) {
					if (l2 == i && i3 >= i1 && i3 < i1 + 8 && j3 >= j1 && j3 < j1 + 8) {
						readTile(regionId, l1 + Class4.method156(j3 & 7, j, i3 & 7), 0, stream, l + Class4.method155(j, j3 & 7, i3 & 7), k1, j, 0);
					} else {
	...
	public final void method180(int regionId, byte abyte0[], int i, int j, int k, int l, CollisionData aclass11[]) {
	...
	private void readTile(int regionId, int i, int j, ByteBuffer stream, int k, int l, int i1, int k1) {
		if (k >= 0 && k < 104 && i >= 0 && i < 104) {
			...
			do {
				...
				if (l1 <= 49) {
					...
					overlayDataType[l][k][i] = OldschoolMaps.isOldschoolRegion(regionId) ? DataType.OLDSCHOOL : DataType.REGULAR;
					...
					underlayDataType[l][k][i] = OldschoolMaps.isOldschoolRegion(regionId) ? DataType.OLDSCHOOL : DataType.REGULAR;
				}
			} while (true);
		}
	...
	public final void readObjectMap(CollisionData aclass11[], WorldController worldController, int i, int j, int k, int l, byte abyte0[], int i1, int j1, int k1
			, int regionId) {
		label0: {
		ByteBuffer stream = new ByteBuffer(abyte0);
		int l1 = -1;
		...
		if (j3 == i && i3 >= i1 && i3 < i1 + 8 && l2 >= k && l2 < k + 8) {
			...
			if (j4 > 0 && k4 > 0 && j4 < 103 && k4 < 103) {
				...
				CollisionData class11 = null;
				if (l4 >= 0) {
					class11 = aclass11[l4];
				}
				addObjectToRenderer(regionId, k4, worldController, class11, l3, l, j4, l1, i4 + j1 & 3);
			}
		}
			} while (true);
		} while (true);
	...
	public final void method190(int regionId, int i, CollisionData aclass11[], int j, WorldController worldController, byte abyte0[]) {
		label0: {
		...
		do {
			...
			int l2 = k2 >> 2;
				...
				if (j3 > 0 && k3 > 0 && j3 < 103 && k3 < 103) {
					...
					addObjectToRenderer(regionId, k3, worldController, class11, l2, j2, j3, l, i3);
				}
			} while (true);
		} while (true);
	}
