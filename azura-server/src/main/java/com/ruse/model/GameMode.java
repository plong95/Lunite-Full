...
public enum GameMode {
	...
	NORMAL, ULTIMATE_IRONMAN, IRONMAN, EXTREME_MODE, GROUP_IRONMAN;//its called extreme mode on heresoz
	...
	public static void set(Player player, GameMode newMode, boolean death) {
		...
		player.getPacketSender().sendInterfaceRemoval();
		if (player.getGameMode() == IRONMAN && newMode != NORMAL && (!(player.getRights().isHighStaff()))) {
		...
		if (newMode != player.getGameMode() || death) {
			/*
			 * if(!player.newPlayer()) { player.getEquipment().resetItems().refreshItems();
			 * player.getInventory().resetItems().refreshItems(); for(Bank b :
			 * player.getBanks()) { b.resetItems(); } player.getSlayer().resetSlayerTask();
			 * player.getSlayer().setSlayerTask(SlayerTasks.NO_TASK).setAmountToSlay(0).
			 * setTaskStreak(0).setSlayerMaster(SlayerMaster.VANNAKA);
			 * player.getSkillManager().newSkillManager(); for(Skill skill : Skill.values())
			 * { player.getSkillManager().updateSkill(skill); } for(AchievementData d :
			 ...
			 * player.getKillsTracker().clear(); player.getDropLog().clear();
			 * player.getPointsHandler().reset(); PlayerPanel.refreshPanel(player);
			 * player.getUpdateFlag().flag(Flag.APPEARANCE); if(player.didReceiveStarter())
			 * { if(newMode != GameMode.NORMAL) { player.getInventory().add(995,
			 * 10000).add(1153, 1).add(1115, 1).add(1067, 1).add(1323, 1).add(1191,
			 * 1).add(841, 1).add(882, 50).add(1167, 1).add(1129, 1).add(1095, 1).add(1063,
			 * 1).add(579, 1).add(577, 1).add(1011, 1).add(1379, 1).add(556, 50).add(558,
			 * 50).add(557, 50).add(555, 50).add(1351, 1).add(1265, 1).add(1007,
			 * 1).add(1061, 1).add(330, 100).add(16127, 1); } else {
			 * player.getInventory().add(995, 500000).add(1153, 1).add(1115, 1).add(1067,
			 * 1).add(1323, 1).add(1191, 1).add(841, 1).add(882, 1000).add(1167,
			 * 1).add(1129, 1).add(1095, 1).add(1063, 1).add(579, 1).add(577, 1).add(1011,
			 * 1).add(1379, 1).add(556, 1000).add(558, 1000).add(557, 1000).add(555,
			 * 1000).add(1351, 1).add(1265, 1).add(1007, 1).add(1061, 1).add(386,
			 * 100).add(16127, 1); } } else { player.getPacketSender().
			 * sendMessage("Your connection has received enough starting items."); } }
			 */
		}
