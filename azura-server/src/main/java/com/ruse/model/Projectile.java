...
public final class Projectile {
	 ...
	 * @param curve        the curve angle of the projectile.
	 */
	public Projectile(Object source, Entity victim, int projectileId, int delay, int speed, int startHeight, int endHeight, int curve) {
		this(
				new Position(
						source instanceof GameObject ? ((GameObject) source).getPosition().getX() : ((Entity) source).getPosition().getX() + ((Entity) source).getSize() / 2,
						source instanceof GameObject ? ((GameObject) source).getPosition().getY() : ((Entity) source).getPosition().getY() + ((Entity) source).getSize() / 2,
						source instanceof GameObject ? ((GameObject) source).getPosition().getZ() : ((Entity) source).getPosition().getZ()),
				victim.getPosition(),
				victim.isPlayer() ? -victim.getIndex() - 1 : victim.getIndex() + 1,
				projectileId, delay, speed, startHeight, endHeight, curve);
	...
	public void sendProjectile() {
		for (Player player : World.getPlayers()) {
			...
			if (start.isViewableFrom(player.getPosition())) {
				player.getPacketSender().sendProjectile(start, offset, 0, speed, projectileId, startHeight, endHeight,
						lockon, delay);
	 ...
	 * 
	 * @param count the amount of projectiles to send.
	 */
