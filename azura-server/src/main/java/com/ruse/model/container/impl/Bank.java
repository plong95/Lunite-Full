...
public class Bank extends ItemContainer {
	...
	public Bank open(Player player, boolean reset) {
		...
		if (getPlayer().getGameMode() == GameMode.ULTIMATE_IRONMAN
				&& getPlayer().getEquipment().getItems()[Equipment.RING_SLOT].getId() == 23660
				...
				|| getPlayer().getGameMode() == GameMode.ULTIMATE_IRONMAN
				&& getPlayer().getEquipment().getItems()[Equipment.AMULET_SLOT].getId() == 23313
				|| getPlayer().getGameMode() == GameMode.ULTIMATE_IRONMAN
				&& getPlayer().getEquipment().getItems()[Equipment.AMULET_SLOT].getId() == 23540
				|| getPlayer().getGameMode() == GameMode.ULTIMATE_IRONMAN
				&& getPlayer().getEquipment().getItems()[Equipment.AMULET_SLOT].getId() == 23541
				|| getPlayer().getGameMode() == GameMode.ULTIMATE_IRONMAN
				&& getPlayer().getEquipment().getItems()[Equipment.AMULET_SLOT].getId() == 23354
				|| getPlayer().getGameMode() == GameMode.ULTIMATE_IRONMAN
		...
		if (getPlayer().getGameMode() == GameMode.ULTIMATE_IRONMAN) {
			...
			if (getPlayer().getGameMode() == GameMode.ULTIMATE_IRONMAN) {
				getPlayer().getPacketSender().sendInterfaceRemoval()
						.sendMessage("@red@You are not able to deposit anything into this bank.");
			}
		}
	...
	public static void depositItems(Player p, ItemContainer from, boolean ignoreReqs) {
		...
		if (!ignoreReqs)
			if (!p.isBanking() || p.getInterfaceId() != 5292)
				return;
		if (p.getInventory().getAmount(995) > 10 && p.getInventory().contains(995)) {
			int coinsamount = p.getInventory().getAmount(995);
			p.getInventory().delete(995,coinsamount);
		...
		for (Item it : from.getValidItems()) {
			...
			if ((toBank.getId() >= 23460 && toBank.getId() <= 23462 )
			 || (toBank.getId() >= 23748 && toBank.getId() <= 23751 )|| (toBank.getId() ==  15111)
			){
				p.getPacketSender().sendMessage("You cannot deposit this item into your bank.");
				continue;
			}
	...
	public static class BankSearchAttributes {
		...
		public static void stopSearch(Player player, boolean openBank) {
			...
			player.getPacketSender().sendString(27002, Integer.toString(0)).sendString(27000, "1").sendConfig(117, 0)
					.sendString(5383, "  The Bank of Lunite");
			if (openBank)
