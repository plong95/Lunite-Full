	...
	public Inventory switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
		...
		if (to instanceof Bank) {
			...
			for (Item it : getValidItems()) {
				if (getPlayer().getBank(getPlayer().getCurrentBankTab()).getFreeSlots() <= 0
						&& !(getPlayer().getBank(getPlayer().getCurrentBankTab()).contains(it.getId()) && it.getDefinition().isStackable())) {
					getPlayer().getPacketSender().sendMessage("Bank full.");
					return this;
				}
			}
		}
