...
public class InviteRaidsPlayer extends Input {
	...
	public void handleSyntax(Player player, String plrToInvite) {
		...
		if (player.getLocation() == Locations.Location.DEATH_SANCTUM_LOBBY) {
			player.getPacketSender().sendInterfaceRemoval();
			Player invite = World.getPlayerByName(plrToInvite);
			if (invite == null) {
				player.getPacketSender().sendMessage("That player is currently not online.");
			...
			if (player.getSanctumOfDeathParty().getPlayers().contains(invite)) {
				player.getPacketSender().sendMessage("That player is already in your party.");
				return;
			}
			player.sendMessage("Sent invite to " + plrToInvite);
			player.getSanctumOfDeathParty().invite(invite);
		}else
		if (player.getLocation() == Locations.Location.ANIMA_CHAMBERS_LOBBY) {
			player.getPacketSender().sendInterfaceRemoval();
			Player invite = World.getPlayerByName(plrToInvite);
			if (invite == null) {
				player.getPacketSender().sendMessage("That player is currently not online.");
			...
			if (player.getChambersOfAnimaParty().getPlayers().contains(invite)) {
				player.getPacketSender().sendMessage("That player is already in your party.");
				return;
			}
			player.sendMessage("Sent invite to " + plrToInvite);
			player.getChambersOfAnimaParty().invite(invite);
		}
	}
}