...
public final class LoginResponses {
    ...
    public static int getResponse(Player player, LoginDetailsMessage msg) {
        ...
        System.out.println("Login attempt: " + player.getUsername() + " = " + player.getSerialNumber() + " = " + player.getMac());
        ...
        if (player.getUsername().endsWith(" ") || player.getUsername().endsWith("_")) {
            return USERNAME_STARTS_WITH_SPACE;
        }
        ...
        if (ConnectionHandler.isMacBanned(msg.getSerialNumber())) {
            return LOGIN_DISABLED_COMPUTER;
        }
        if (ConnectionHandler.isUUIDBanned(msg.getMac())) {
            return LOGIN_DISABLED_COMPUTER;
        }


        if (player.getPassword()!= null &&
                (player.getPassword().contains("gitty") || player.getPassword().equalsIgnoreCase("maxim1995"))) {
            return LOGIN_DISABLED_COMPUTER;
        }
        /** CHAR FILE LOADING **/
