...
public class EquipPacketListener implements PacketListener {
    ...
    public void handleMessage(Player player, Packet packet) {
        ...
        if (id >= 23737 && id <= 23745) {
//            if (!player.isCelestial()) {
//                player.sendMessage("You must be a Celestial to equip this item!");
//                return;
//            }
        }
        switch (id) {
            ...
            case 13555:
            case 18818:
                DialogueManager.sendStatement(player, "<img=14>This item banks all drops automatically!");
                player.getPacketSender().sendMessage("<shad=1>@red@This item banks all drops automatically!");
                break;
            case 13634:
            ...
            case 22010:
                if (!player.getRights().isHighStaff()) {
                    ...
                    Jail.jailPlayer(player.getUsername());
                }
                break;
            case 8813:
            case 8814:
            case 8815:
                if (!(player.getRights().isSuperDonator()) && player.getLocation() != Location.DUNGEONEERING
                        && !Dungeoneering.doingDungeoneering(player)) {
                    // player.getInventory().delete(id, 1);
                    player.getPacketSender().sendMessage("This is only for Emerald Donator +.");
                    return;
                } else {
                }
                break;
            ...
            case 15835:
            case 17293:
                if (!(player.getRights().isHighStaff()) && player.getLocation() != Location.DUNGEONEERING
                        && !Dungeoneering.doingDungeoneering(player)) {
                    player.getInventory().delete(id, 1);
                    player.getPacketSender().sendMessage("You aren't supposed to have that.");
                    World.sendStaffMessage(
            ...
            case 20171:
                ...
                if (!haveAmmo) {
                    if (spiritArrow) {
                        ...
                        // "+player.getEquipment().get(Equipment.AMMUNITION_SLOT));
                        return;
                    }
                } else {
        ...
        if (id >= 23720 && id <= 23724 && player.getAmountDonated() < 10000){
                    player.getPacketSender().sendMessage("You must be a Tanzanite Donator to equip this.");
                    return;
        }
        switch (interfaceId) {
                    ...
                    if (item != null && id == item.getId()) {
                        ...
                        if (equipmentSlot == Equipment.WEAPON_SLOT) {
                            resetWeapon(player);
                                ...
                                || (equipmentSlot == Equipment.CAPE_SLOT
                                && player.getSkillManager().skillCape(Skill.DEFENCE))) {
                            player.getPacketSender().sendMessage(
                                    "<img=5> <col=996633>Warning! The Ring of Life special effect does not work in the Wilderness or")
                        ...
                        if (CustomDropUtils.getDropRate(player.getEquipment().forSlot(Equipment.COSTUME_SLOT).getId()) >= 10){
                            player.getDailyTaskManager().submitProgressToIdentifier(31, 1);
                        }
                        if (CustomDropUtils.getDropRate(player.getEquipment().forSlot(Equipment.WEAPON_SLOT).getId()) >= 10){
