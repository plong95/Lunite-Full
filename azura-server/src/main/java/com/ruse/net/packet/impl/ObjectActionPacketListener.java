    ...
    private static void firstClick(final Player player, Packet packet) {
        ...
        player.setWalkToTask(new WalkToTask(player, position, gameObject.getSize(), new FinalizedMovementTask() {
            ...
            public void execute() {
                ...
                if (gameObject.getId() == HallsOfAmmo.CHEST_ID) {
                    HallsOfAmmo.handleObject(player);
                }
                ...
                if (ConstructionActions.handleFirstObjectClick(player, gameObject)) {
                    return;
                        ...
                        /*if (gameObject.getDefinition() != null && gameObject.getDefinition().getName() != null
                                ...
                                && player.getRights().isHighStaff()) {
                            player.getPacketSender().sendMessage("You just clicked a door. ID: " + id);
                            // CustomObjects.deleteGlobalObject(gameObject);

                            *//*
                 * Door door = Door.create(gameObject.getId(), gameObject.getPosition().getX(),
                 * gameObject.getPosition().getY()); //GameObject obj =
                 * Region.loadRegion(gameObject.getPosition().getX(),
                 * gameObject.getPosition().getY()).getObject(gameObject.getId(),
                 * gameObject.getPosition().getX(), gameObject.getPosition().getY());
                 * //player.createObject(gameObject.getPosition().getX(),
                 * gameObject.getPosition().getY(), gameObject.getId(), door.isOpen() ?
                 * obj.getFace() : obj.getFace() + 1, 0);
                 * player.getPacketSender().sendObject(new GameObject(gameObject.getId(), new
                 * Position(gameObject.getPosition().getX(), gameObject.getPosition().getY()),
                 * 10, (door.isOpen() ? gameObject.getFace() : gameObject.getFace() + 1)));
                 * door.setOpen(!door.isOpen());
                 *//*
                        }*/


                if (RaidsManager.handleFirstClickObjectInteraction(player, gameObject))
                    return;
                ...
                switch (id) {
                    case 7617:
                        ...
                        if (player.getInventory().contains(2126)) {
                            player.getPacketSender().sendMessage("You already have dwellberries.");
                            return;
                        }
                        if (player.getCurseOfArrav().getQuestProgress() == 1) {
                            player.performAnimation(new Animation(827));
                            player.getInventory().add(2126, 1);
                            player.getPacketSender().sendMessage("You pick some Dwellberries.");
                            player.getClickDelay().reset();
                        }
                        break;
                    case 37:
                        if (gameObject.getPosition().getY() == 9708 && player.getPosition().getY() < gameObject.getPosition().getY())
                            player.getCurseOfArrav().openGate();
                        break;
                    case 38828:
                        if (player.getLocation() == Location.CELESTIAL_ZONE)
                            CelestialZone.feedFire(player);
                        break;
                    case 4389:
                        CelestialZone.celestialPortal(player);
                        break;
                    case 12006:
                        if (player.getLocation() == Location.CELESTIAL_ZONE)
                            CelestialZone.pickMushroom(player, gameObject);
                        break;
                    case 54884:
                        if (player.getLocation() == Location.CELESTIAL_ZONE)
                            CelestialZone.smeltBar(player);
                        break;
                    case 10437:
                        RaffleInterface.openInterfacePerks(player);
                        break;
                    case 42650:
                        EggSpawns.claimEgg(player, gameObject);
                        break;
                    case 7256:
                        player.getAssassinsGuild().handleDoors(gameObject);
                        break;
                    case 49921:
                    ...
                    case 29577:
                        ...
                        ChambersOfAnimaRewards.openInterface(player);
                        break;
                    case 21879:
                        if (player.getInventory().getAmount(23460) >= 10) {
                            player.getInventory().delete(23460, 10);
                            player.getInventory().add(23461, 1);
                        } else {
                            player.sendMessage("You need 10 Patronum ores to make a bar.");
                        }
                        break;
                    case 22725:
                        if (player.getInventory().getAmount(23461) >= 3) {
                            player.getInventory().delete(23461, 3);
                            player.getInventory().add(23462, 1);
                            player.getVotingMinigame().startWave();
                        } else {
                            player.sendMessage("You need 3 Patronum bars to make a Patronum blade.");
                        }
                        break;
                    case 4004:
                        LuniteGuardian.sacrificeTicketsInput(player);
                        break;
                    case 41207:
                        ...
                        break;
                    case 2471:
                        if (player.getInventory().contains(19094) || player.getRights() == PlayerRights.DEVELOPER) {
                            player.getInventory().delete(19094, 1);
                            TeleportHandler.teleportPlayer(player, new Position(2523, 2505, 8),
                                    player.getSpellbook().getTeleportType());
                            player.sendMessage("<shad=1>@yel@Teleporting to the@bla@ Second Room!");
                        } else {
                            player.sendMessage(
                                    "<shad=1>@red@You don't have the Cash Room Key, which is needed to teleport to the Second Room!");
                            return;
                        }
                        break;
                    case 2472:

                        if (player.getLocation() == Location.WAVE_MINIGAME_LOBBY)
                            player.getWaveMinigame().startMinigame();
                        else if (player.getLocation() == Location.WAVE_MINIGAME)
                            player.getWaveMinigame().handleDeath();
                               /* if (player.getInventory().contains(19094) || player.getRights() == PlayerRights.DEVELOPER) {
                                    player.getInventory().delete(19094, 1);
                                    TeleportHandler.teleportPlayer(player, new Position(2527, 2526, 12),
                                            player.getSpellbook().getTeleportType());
                                    player.sendMessage("<shad=1>@yel@Teleporting to the@bla@ Third Room!");
                                } else {
                                    player.sendMessage(
                                            "<shad=1>@red@You don't have the Cash Room Key, which is needed to teleport to the Third Room!");
                                    return;
                                }*/
                        break;
                        /*    case 2473:
                                if (player.getInventory().contains(19094) || player.getRights() == PlayerRights.DEVELOPER) {
                                    player.getInventory().delete(19094, 1);
                                    TeleportHandler.teleportPlayer(player, new Position(2510, 2524, 16),
                                            player.getSpellbook().getTeleportType());
                                    player.sendMessage("<shad=1>@yel@Teleporting to the@bla@ Final Room!");
                                } else {
                                    player.sendMessage(
                                            "<shad=1>@red@You don't have the Cash Room Key, which is needed to teleport to the Final Room!");
                                    return;
                                }
                                break;*/
                    ...
                    case 2914:
                    case 2912:
                        ProgressionZone.handleGates(player, gameObject);
                        break;
                    case 7289:
                    ...
                    case 10820:
                        if (player.getChambersOfAnimaParty() != null)
                            ChambersOfAnima.handlePortal(player.getChambersOfAnimaParty(), player, gameObject);
                        else if (player.getSanctumOfDeathParty() != null)
                            DeathSanctum.handlePortal(player.getSanctumOfDeathParty(), player, gameObject);
                        else
                        ...
                        if (player.getLocation() == Location.ZOMBIE_LOBBY) {
                            if (player.getZombieParty() != null) {
                                if (player.getZombieParty().getOwner().equals(player)) {
                                    ...
                                    player.sendMessage("Only the party leader can start the Island Raids");
                                }
                            } else {
                                player.sendMessage("You must be in a party to start the Island Raids");
                            }
                        }
                        break;
                    case 38150:
                        ...
                        if (player.getLocation() == Location.DEATH_SANCTUM_LOBBY) {
                            if (player.getSanctumOfDeathParty() != null) {
                                if (player.getSanctumOfDeathParty().getOwner().equals(player)) {
                                    player.setDialogueActionId(2013);
                                    DialogueManager.start(player, 2013);
                                } else {
                                    player.sendMessage("Only the party leader can start the Sanctum of Death");
                                }
                            } else {
                                player.sendMessage("You must be in a party to start the Sanctum of Death");
                            }
                        }
                        break;
                    case 38694:
                        ...
                        if (player.getLocation() == Location.ANIMA_CHAMBERS_LOBBY) {
                            if (player.getChambersOfAnimaParty() != null) {
                                if (player.getChambersOfAnimaParty().getOwner().equals(player)) {
                                    ...
                                    player.sendMessage("Only the party leader can start the Chambers of Anima");
                                }
                            } else {
                                player.sendMessage("You must be in a party to start the Chambers of Anima");
                            }
                        }
                        break;
                    case 52601:
                    ...
                    case 22769:
                        if (player.getAmountDonated() >= 10000)
                            ...
                            player.sendMessage("You must be a Tanzanite Donator to do this.");
                        break;
                    case 22772:
                        if (player.getAmountDonated() >= 25000)
                            Stalls.stealFromAFKStall(player, id, 6);
                        else
                            player.sendMessage("You must be a Hydrix Donator to do this.");
                        break;
                    case 13291:
                    ...
                    case 59731:
                        ...
                        if (player.getInventory().contains(13591, 1)) {
                            ...
                            player.getPacketSender().sendItemOnInterface(29132, reward.getId(), 0,
                                    reward.getAmount());
                            int[] Items1 = new int[]{12855, 5022, 5021};
                            ...
                            //  player.getCollectionLogManager().addItem(CollectionLogs.RAIDS_1, reward);
                    ...
                    case 172:
                        ...
                        int[] rarepvm = new int[]{6571, 11137, 3905, 20535, 17704, 11716, 3321, 3319,
                ...
                .setWalkToTask(new WalkToTask(player, position, gameObject.getSize(), () -> {
                    ...
                    switch (gameObject.getId()) {
                        ...
                        case 5917: // friday the 13th event
                            Stalls.stealFromStall(player, 1, 0, 13150,
                        ...
                        case 4878:
                            ...
                            int[] RewardId1111 = new int[]{12855};
        ...
        player.setWalkToTask(new WalkToTask(player, position, gameObject.getSize(), new FinalizedMovementTask() {
            ...
            public void execute() {
                switch (finalId) {
                    ...
                    case 41207:
                        PossibleLootInterface.openInterface(player, PossibleLootInterface.LootData.TH);
                        break;
                }
                Construction.handleFifthObjectClick(x, y, finalId, player);
            }
        }));
    }
    ...
    private static void fifthClick(final Player player, Packet packet) {
        ...
        player.setWalkToTask(new WalkToTask(player, position, gameObject.getSize(), new FinalizedMovementTask() {
            ...
            public void execute() {
                switch (id) {
                    case 22769:
                        PossibleLootInterface.openInterface(player, PossibleLootInterface.LootData.TANZ_STALL);
                        break;
                    case 22772:
                        PossibleLootInterface.openInterface(player, PossibleLootInterface.LootData.HYDRIX_STALL);
                        break;
                    case 52601:
                        PossibleLootInterface.openInterface(player, PossibleLootInterface.LootData.AFK1);
                        break;
                    case 53654:
                        PossibleLootInterface.openInterface(player, PossibleLootInterface.LootData.AFK2);
                        break;
                    case 30035:
                        PossibleLootInterface.openInterface(player, PossibleLootInterface.LootData.AFK3);
                        break;
                }
