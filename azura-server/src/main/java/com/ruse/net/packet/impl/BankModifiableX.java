...
public class BankModifiableX implements PacketListener {
	...
	public void handleMessage(Player player, Packet packet) {
		...
		case 5064:
			...
			if (!player.isBanking() || storing.getId() != item || !player.getInventory().contains(storing.getId())
		...
		case 5382:
			...
			int outcome = player.getBankSearchingAttribtues().isSearchingBank()
					&& player.getBankSearchingAttribtues().getSearchedBank() != null
							? player.getBankSearchingAttribtues().getSearchedBank().getItems()[slot].getId()
							: player.getBank(tab).getItems()[slot].getId();
