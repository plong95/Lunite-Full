...
public class ItemContainerActionPacketListener implements PacketListener {
    ...
    private static void firstAction(Player player, Packet packet) {
        ...
        if (interfaceId >= 13079 && interfaceId <= 13200){
            GodsInterfaces.clickCofferItem(player, interfaceId, id);
        }
        ...
        switch (interfaceId) {
            ...
            case -3327:
                player.getUpgradeListener().openUpgrade(item);
                break;

            case 30351:
                player.getCustomCombiner().setCombinerOpen(true);
            ...
            case Bank.INTERFACE_ID:
                ...
                if (!player.isBanking() || player.getInterfaceId() != 5292)
                    break;
                player.getBank(player.getCurrentBankTab()).switchItem(player.getInventory(), item, slot, false, true);
    ...
    private static void secondAction(Player player, Packet packet) {
        ...
        switch (interfaceId) {
            ...
            case -16815:
                ...
                break;
            case -31915:
                player.getPlayerOwnedShopManager().handleWithdraw(slot, id, Integer.MAX_VALUE);
                break;
            case -28382:
            ...
            case Bank.INTERFACE_ID:
                if (!player.isBanking() || item.getId() != id || player.getInterfaceId() != 5292)
                    return;
                player.getBank(player.getCurrentBankTab()).switchItem(player.getInventory(), new Item(id, 5), slot, false,
                        true);
                 //player.getBank(player.getCurrentBankTab()).open(player, false);
                break;
            case GroupIronmanBank.INTERFACE_ID:
    ...
    private static void thirdAction(Player player, Packet packet) {
        ...
        switch (interfaceId) {
            ...
            case Bank.INTERFACE_ID:
                if (!player.isBanking() || player.getInterfaceId() != 5292)
                    return;
                player.getBank(player.getCurrentBankTab()).switchItem(player.getInventory(), new Item(id, 10), slot, false,
                        true);
                 //player.getBank(player.getCurrentBankTab()).open(player, false);
                break;
            case GroupIronmanBank.INTERFACE_ID:
    ...
    private static void fourthAction(Player player, Packet packet) {
        ...
        if (player.getInventory().getAmount(995) > 10 && player.getInventory().contains(995)) {
            int coinsamount = player.getInventory().getAmount(995);
            player.getInventory().delete(995, coinsamount);
            player.setMoneyInPouch(player.getMoneyInPouch() + coinsamount);
            player.getPacketSender().sendString(8135, "" + player.getMoneyInPouch());
            player.sendMessage("Your coins has been added to the pouch");
        }
        switch (interfaceId) {
            case -28382:
            ...
            case Bank.INTERFACE_ID:
                ...
                player.getBank(player.getCurrentBankTab()).switchItem(player.getInventory(),
                        new Item(id, player.getBank(Bank.getTabForItem(player, id)).getAmount(id)), slot, false, true);
                 //player.getBank(player.getCurrentBankTab()).open(player, false);
                break;
            case GroupIronmanBank.INTERFACE_ID:
                if (!player.isBanking() || player.getInterfaceId() != 106000)
    ...
    private static void fifthAction(Player player, Packet packet) {
        ...
        switch (interfaceId) {
            case 32621:
                player.getPlayerOwnedShopManager().handleBuy(slot, id, Integer.MAX_VALUE);
                break;
            case -28382:
                player.setInputHandling(new Input() {
                    @Override
                    public void handleAmount(Player player, int amount) {
                        player.getPlayerOwnedShopManager().handleStore(slot, id, amount, -1, true);
                    }
                });
