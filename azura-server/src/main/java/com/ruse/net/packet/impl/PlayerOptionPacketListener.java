...
public class PlayerOptionPacketListener implements PacketListener {
	...
	private static void attack(Player player, Packet packet) {
		...
		RaidsManager.handleRightClickInvite(player, attacked);
		...
		if (player.getLocation() == Location.DEATH_SANCTUM_LOBBY) {
			player.setEntityInteraction(attacked);
			if (attacked.getIndex() != player.getIndex()) {
				if (player.getSanctumOfDeathParty() != null && player.getSanctumOfDeathParty().getOwner().equals(player)) {
					player.sendMessage("Sent invite to " + attacked.getUsername());
					player.getSanctumOfDeathParty().invite(attacked);
				} else {
					player.sendMessage("You must be the leader of a party to do this.");
				}
			}
		...
		if (player.getLocation() == Location.ANIMA_CHAMBERS_LOBBY) {
			player.setEntityInteraction(attacked);
			if (attacked.getIndex() != player.getIndex()) {
				if (player.getChambersOfAnimaParty() != null && player.getChambersOfAnimaParty().getOwner().equals(player)) {
					player.sendMessage("Sent invite to " + attacked.getUsername());
					player.getChambersOfAnimaParty().invite(attacked);
				} else {
					player.sendMessage("You must be the leader of a party to do this.");
				}
			}
