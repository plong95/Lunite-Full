...
import com.ruse.world.content.youtube.YoutubeChannelInfo;
import com.ruse.world.content.youtube.YoutubeMgr;
import com.ruse.world.content.youtube.YoutubeVideoDetails;
...
public class PacketSender {
    ...
    public PacketSender sendYoutubeData() {
        sendYoutubeVideos();
        sendYoutubeChannels();
        return this;
    }
    ...
    public PacketSender sendYoutubeVideos() {
        PacketBuilder out = new PacketBuilder(192, PacketType.SHORT);
        out.putString("video");
        out.putShort(YoutubeMgr.getVideos().size() >= 15 ? 15 : YoutubeMgr.getVideos().size());
        int size = 0;
        for (YoutubeVideoDetails video : YoutubeMgr.getVideos()) {
            if (size < 15) {
                out.putString(video.getVideoId());
                out.putString(video.getTitle());
                out.putShort((short) video.getViewCount());
                out.putShort((short) video.getLikeCount());
                out.putShort((short) video.getCommentCount());
                out.putString(video.getChannelName());
                out.putString(video.getChannelId());
                out.putString(video.getThumbnailUrl());

                SimpleDateFormat month_date = new SimpleDateFormat("MMM dd hh:mm aa", Locale.ENGLISH);
                Date date = video.getPublishedDate();
                String month_name = month_date.format(date);
                out.putString(month_name);
    ...
    public PacketSender sendYoutubeChannels() {
        PacketBuilder out = new PacketBuilder(192, PacketType.SHORT);
        out.putString("channel");
        out.putShort(YoutubeMgr.getChannels().size());
        for (YoutubeChannelInfo channel : YoutubeMgr.getChannels()) {
            out.putString(channel.getChannelId());
            out.putString(channel.getName());
            out.putLong(channel.getSubscribers());
            out.putString(channel.getProfilePic());
        }
        player.getSession().queueMessage(out);
        return this;
    }
    ...
    public void updateInterfaceVisibility(int interfaceId, boolean visible) {
        ...
        out.putInt(interfaceId).put(visible ? 1 : 0);
        player.getSession().queueMessage(out);
    }
    ...
    public PacketSender sendEffectTimer(int seconds, int itemID) {
        PacketBuilder out = new PacketBuilder(54);
        out.putShort(seconds);
        out.putInt(itemID);
        player.getSession().queueMessage(out);
    ...
    public PacketSender sendQuickPrayerStatus(boolean active) {
    ...
    public PacketSender sendTabs() {
        ...
        sendTabInterface(GameSettings.ACHIEVEMENT_TAB, 111000);
    ...
    public PacketSender sendDuelEquipmentOther(Player target) {
        for (int i = 0; i < target.getEquipment().getItems().length; i++) {
            PacketBuilder out = new PacketBuilder(34, PacketType.SHORT);
            out.putShort(13824);
            out.put(i);
            out.putShort(target.getEquipment().getItems()[i].getId() + 1);
            out.put(255);
            out.putInt(target.getEquipment().getItems()[i].getAmount());
            player.getSession().queueMessage(out);
        }
        return this;
    }
