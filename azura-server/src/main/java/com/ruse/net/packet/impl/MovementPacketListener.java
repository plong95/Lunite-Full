...
public class MovementPacketListener implements PacketListener {
	...
	public void handleMessage(Player player, Packet packet) {
		...
		if (player.getScratchcard() != null && player.getScratchcard().getGame().isGameStarted()
		&& player.getInterfaceId() == 25400){
			player.sendMessage("You can't move until you claim your prize.");
			return;
		}
		...
		if (player.isClickToTeleport() && steps > 0) {
			player.moveTo(new Position(path[steps - 1][0] + firstStepX, path[steps - 1][1] + firstStepY,
	...
	public boolean checkReqs(Player player, int opcode) {
		...
		if (player.isPlayerLocked() && (player.getHasPin() == true && !player.getSavedIp().equalsIgnoreCase(player.getHostAddress()))) {
