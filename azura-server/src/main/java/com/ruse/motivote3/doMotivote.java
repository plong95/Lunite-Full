...
public class doMotivote implements Runnable {
		...
		SERVICE.execute(() -> {
			try {
				...
				if (success) {
					...
					AchievementsOLD.doProgress(player, VOTE_100_TIMES, 1);
	...
	/*
	 * Thread one = new Thread() { public void run() { try { boolean success =
	 * motivote.redeemVote(auth); Item item = new Item(19670, 1); if (success) {
	 * player.getInventory().add(item, true);
	 * player.getPacketSender().sendMessage("Auth redeemed, thanks for voting!");
	 * player.getLastVoteClaim().reset(); Achievements.doProgress(player,
	 * VOTE_100_TIMES, 1); voteCount ++; if (voteCount >=
	 * GameSettings.Vote_Announcer) { World.
	 * sendMessage("<img=5><shad=0><col=bb43df> 10 more players have just voted! Use ::vote for rewards! Thanks, <col="
	 * +player.getYellHex()+">"+player.getUsername()+"<col=bb43df>!"); voteCount =
	 * 0; } else { player.getPacketSender().
	 * sendMessage("<img=5><shad=0><col=bb43df>Thank you for voting and supporting Necrotic!"
	 * ); } } else { player.getPacketSender().
	 * sendMessage("Invalid voting auth supplied, please try again.");
	 * player.getLastVoteClaim().reset(); } } catch (Exception ex) {
	 * ex.printStackTrace(); } } }; one.start(); }
	 */
