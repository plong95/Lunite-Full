...
public class AngelicPotionTask extends Task {

	public AngelicPotionTask(Player player) {
		super(1, player, true);
		this.player = player;
	}
	...
	public void execute() {
		...
		if (player.getOverloadPotionTimer() == 20)
			player.getPacketSender().sendMessage("@red@Your Angelic Potion's effect is about to run out.");
		if (player.getOverloadPotionTimer() <= 0 || player.getLocation() == Location.DUEL_ARENA
			...
			player.getPacketSender().sendMessage("@red@Your Angelic Potion's effect has run out.");
			for (int i = 0; i < 7; i++) {
				if (i == 3 || i == 5)
					continue;
