...
public class NPCUpdating {
	...
	public static void update(Player player) {
		...
		for (Iterator<NPC> npcIterator = player.getLocalNpcs().iterator(); npcIterator.hasNext(); ) {
			...
			boolean hideNpc = !player.isRenderPets() && BossPets.BossPet.forSpawnId(npc.getId()) != null;
			if (World.getNpcs().get(npc.getIndex()) != null && npc.isVisible()
					&& player.getPosition().isWithinDistance(npc.getPosition()) && !npc.isNeedsPlacement() && !hideNpc) {
		...
		for (NPC npc : World.getNpcs()) {
			if (player.getLocalNpcs().size() >= 255 || number >= 24) // Originally 255
				break;
			boolean hideNpc = npc == null || (!player.isRenderPets() && BossPets.BossPet.forSpawnId(npc.getId()) != null);
			if (npc == null || player.getLocalNpcs().contains(npc) || !npc.isVisible() || npc.isNeedsPlacement() || hideNpc)
				continue;
	...
	private static void updateSingleHit(PacketBuilder builder, NPC npc) {
		builder.putInt((int) npc.getPrimaryHit().getDamage());
	...
	private static void updateDoubleHit(PacketBuilder builder, NPC npc) {
		builder.putInt((int) npc.getSecondaryHit().getDamage());
