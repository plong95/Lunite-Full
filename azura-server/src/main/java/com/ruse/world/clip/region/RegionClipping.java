...
public final class RegionClipping {
	...
	public void removeClip(int x, int y, int height, int shift) {
		...
		//getCollisionMap().flag(height, x, y, shift);
	}
	...
	public void addClip(int x, int y, int height, int shift) {
		...
		//getCollisionMap().unflag(height, x, y, shift);
	}
	...
	public static void init() {
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				regionArray[i] = new RegionClipping(regionId, stream.getUShort(), stream.getUShort());
				regionArray[i].clips =  new int[4][][];
				int finalI = i;
				regions.computeIfAbsent(regionId, $ -> regionArray[finalI]);
			}
	...
	private static void loadMaps(int regionId, ByteStream objectStream, ByteStream groundStream) {
		...
		while ((incr = objectStream.getUSmart()) != 0) {
			...
			while ((incr2 = objectStream.getUSmart()) != 0) {
				...
				if (height >= 0 && height <= 3)
					addObject(objectId, absX + localX, absY + localY, height, type, direction); // Add object to
																								// clipping
			}
		}
	}
	...
	public static RegionClipping getRegion(int absoluteX, int absoluteY) {
		return get((absoluteX >> 6) << 8 | (absoluteY >> 6));
	}
	public static RegionClipping getRegion(int id) {
		return  get(id);
	}
	...
	private int getClip(int x, int y, int height) {
		...
		int regionAbsY = (id & 0xff) * 64;
		loadRegion(x, y);
		if (getCollisionMap().getMasks()[height] == null) {
			getCollisionMap().getMasks()[height] = new int[64][64];
		}
		return getCollisionMap().getMasks()[height][x - regionAbsX][y - regionAbsY];*/
	}
	...
	public static boolean canWalk(int level, int x, int y, DirectionFace direction, int size) {
	...
	public static int getCollisionMask(int level, int x, int y) {
		RegionClipping region = getRegion((x >> 6) << 8 | (y >> 6));
		if (region == null) {
			return -1;
		}
