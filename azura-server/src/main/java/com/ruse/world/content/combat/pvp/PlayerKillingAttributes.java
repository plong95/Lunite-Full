...
public class PlayerKillingAttributes {
	...
	private void handleReward(Player o, boolean targetKilled) {
		...
		!player.getHostAddress().equalsIgnoreCase(o.getHostAddress()) && player.getLocation() == Location.WILDERNESS) {
			...
			AchievementsOLD.doProgress(player, AchievementDataOLD.DEFEAT_10_PLAYERS);
			AchievementsOLD.doProgress(player, AchievementDataOLD.DEFEAT_30_PLAYERS);
			...
			if (this.playerKillStreak >= 3) {
				AchievementsOLD.finishAchievement(player, AchievementDataOLD.REACH_A_KILLSTREAK_OF_3);
			}
			if (this.playerKillStreak >= 6) {
				AchievementsOLD.finishAchievement(player, AchievementDataOLD.REACH_A_KILLSTREAK_OF_6);
			}
