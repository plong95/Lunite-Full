...
public final class EvilTree {
				...
				player.setCurrentTask(new Task(1, player, false) {
					...
					public void execute() {
						...
						if (cycle != reqCycle) {
							...
							if (reward == Trees.OAK) {
								AchievementsOLD.finishAchievement(player, AchievementDataOLD.CUT_AN_OAK_TREE);
							} else if (reward == Trees.MAGIC) {
								AchievementsOLD.doProgress(player, AchievementDataOLD.CUT_100_MAGIC_LOGS);
								AchievementsOLD.doProgress(player, AchievementDataOLD.CUT_5000_MAGIC_LOGS);
							}
	...
	public static enum EvilTreeDef {
		...
		// EVIL_WILLOW_TREE("Willow Evil Tree", 11441, 1519, 30, 15, 30, 400, 45, 66,
		// 450),
		// EVIL_MAPLE_TREE("Maple Evil Tree", 11444, 1517, 45, 22, 45, 500, 55, 121,
		// 675),
		// EVIL_YEW_TREE("Yew Evil Tree", 11916, 1515, 60, 30, 60, 650, 64, 172, 1012),
		// EVIL_MAGIC_TREE("Magic Evil Tree", 11919, 1513, 75, 37, 75, 800, 70, 311,
		// 1517),
		...
		private EvilTreeDef(String treeName, int id, int log, int woodcuttingLevel, int farmingLevel,
				int firemakingLevel, int maxCutAmount, int woodcuttingXp, int farmingXp, int firemakingXp) {
	...
	public static void sequence() {
		// // System.out.println("SPAWNED_TREE == null ? "+(boolean) (SPAWNED_TREE ==
		// null)+".");
		if (SPAWNED_TREE == null) {
			if (timer.elapsed(TIME) || firstTime) {
				...
				World.sendMessage("<img=5> <shad=1><col=FF9933>An Evil tree has spawned " + locationData.clue + ".");
				// new DiscordManager(DiscordConstant.EVENTS_CHANNEL, "Evil Tree", "An Evil tree
				// has spawned "+locationData.clue+".").log1();
				World.getPlayers().forEach(p -> PlayerPanel.refreshPanel(p));
				// World.getPlayers().forEach(p -> p.getPacketSender().sendString(39162,
				// "@or2@Crashed star:
				// @yel@"+ShootingStar.CRASHED_STAR.getStarLocation().playerPanelFrame+""));
				timer.reset();
			}
		} else {
	...
	public static void despawn(boolean respawn) {
		...
		if (SPAWNED_TREE != null) {
			...
			// new DiscordManager(DiscordConstant.EVENTS_CHANNEL, "Evil Tree", "The evil
			// tree has despawned, 'we'll get him next time bois"+".").log1();
			SPAWNED_TREE = null;
			for (Player p : World.getPlayers()) {
				if (p == null) {
					continue;
