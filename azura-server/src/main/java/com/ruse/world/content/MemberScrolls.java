...
public class MemberScrolls {
        ...
        switch (item) {
            ...
            case 9082:// custom pet donation
                ...
                if (player.getInventory().getFreeSlots() > 3) {
                    ...
                    // player.getPointsHandler().setMemberPoints(100, true);
    ...
    public static boolean convert(Player player, int item) {
        int amountInInventory = player.getInventory().getAmount(item);
        int amountToConvert;
        int idToGive;
        int amountToGive;
        ...
        switch (item) {
            case 10946:
                converting = true;
                idToGive = 6769;
                amountToConvert = amountInInventory - (amountInInventory % 5);
                amountToGive = amountToConvert / 5;
                break;
            case 6769:
                converting = true;
                idToGive = 10942;
                amountToConvert = amountInInventory - (amountInInventory % 2);
                amountToGive = amountToConvert / 2;
                break;
            case 10942:
                converting = true;
                idToGive = 10935;
        ...
        if (converting) {
            player.getInventory().delete(item, amountToConvert);
            player.getInventory().add(idToGive, amountToGive);
			player.sendMessage("You successfully convert "+amountToConvert+" "+ItemDefinition.forId(item).getName()+" to "+amountToGive+" "+ItemDefinition.forId(idToGive).getName());
            return true;
        } else {
            return false;
        }
    ...
    public static void claimBond(Player player) {
        ...
        if (player.getInventory().contains(item)) {
            ...
            int funds = item == 10946 ? 1
                    : item == 6769 ? 5
                    : item == 10942 ? 10 : item == 10934 ? 25 : item == 10935 ? 50 : item == 10943 ? 100 : -1;
