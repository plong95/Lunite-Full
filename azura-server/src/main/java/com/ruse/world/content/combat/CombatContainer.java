    ...
    private final ContainerHit[] prepareHits(int hitAmount) {
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            if (attacker.isPlayer()) {
                if (victim.isNpc()) {
                    NPC npc = (NPC) victim;
                    if (NPCDrops.globalNpcs.contains(npc.getId()) || npc.getId() == 9017 || npc.getId() == 3305 || npc.getId() == 9014) {
                        if (array[i].getHit().getDamage() >= npc.getDefaultConstitution() / 100)
                            array[i].getHit().setDamage(npc.getDefaultConstitution() / 100);
                    }
                    if (npc.getLocation() == Locations.Location.WAVE_MINIGAME) {
                        if (array[i].getHit().getDamage() >= npc.getDefaultConstitution() / 5)
                            array[i].getHit().setDamage(npc.getDefaultConstitution() / 5);
                    }
                    if (npc.getLocation() == Locations.Location.DEATH_SANCTUM) {
                        if (array[i].getHit().getDamage() >= 1000000)
                            array[i].getHit().setDamage(1000000);
                    }
                    if (npc.getLocation() == Locations.Location.ANIMA_CHAMBERS) {
                        if (array[i].getHit().getDamage() >= 2500000)
                            array[i].getHit().setDamage(2500000);
                    }
                    if (npc.getLocation() == Locations.Location.PEST_CONTROL_GAME) {
                        if (array[i].getHit().getDamage() >= 2500000)
                            array[i].getHit().setDamage(2500000);
                    }
                    if (npc.getLocation() == Locations.Location.ISLE_GODS) {
                        if (array[i].getHit().getDamage() >= 10000000)
                            array[i].getHit().setDamage(10000000);
                    }

                    if (npc.getLocation() == Locations.Location.VORKATH) {
                        if (array[i].getHit().getDamage() >= 7500000)
                            array[i].getHit().setDamage(7500000);
                    }
                    ...
                    if (npc.getId() == 10000
                        ...
                        if (player.getGodsRaidsParty() != null) {
                            if ((!player.getGodsRaidsParty().isSaradominAttackable() && npc.getId() == 10000)
                                    || (player.getGodsRaidsParty().isSaradominAttackable() && npc.getId() == 10001)) {
                    ...
                    if (player.getBossFight() != null && player.getBossFight() instanceof Leviathon) {
                        if (victim.isNpc()) {
                            NPC boss = (NPC) victim;
                            Leviathon t = (Leviathon) player.getBossFight();
                            if (boss.getId() == 28060) {
                                int stage = player.getBossFight().getCurrentStage();
                                if (stage == 3 && t.getKilledMinions() < 3) {
                                    array[i].getHit().setDamage(0);
                                }
                            }
                        }
                    ...
                    if (player.getChambersOfAnimaParty() != null) {
                        if (victim.isNpc()) {
                            ChambersOfAnimaParty raidParty = player.getChambersOfAnimaParty();
                            if (combatType != raidParty.getAcceptableType()) {
                                victim.heal((int) array[i].getHit().getDamage() / 3);
                                array[i].getHit().setDamage(0);
                                if (raidParty.getDifficulty() == ChambersOfAnimaParty.ChambersDifficulty.HARD) {
