...
public class SmithingData {
	...
	public static boolean hasOres(Player player, int barId) {
		...
		if (player.getOres()[0] > 0 && player.getOres()[1] < 0) {
			...
			if (player.getInventory().getAmount(player.getOres()[1]) > 0
					&& player.getInventory().getAmount(player.getOres()[0]) > 0)
				return true;
		} else if (player.getOres()[1] > 0 && player.getOres()[1] == 453 && player.getOres()[0] > 0) {
			if (player.getInventory().getAmount(player.getOres()[1]) >= getCoalAmount(barId)
					&& player.getInventory().getAmount(player.getOres()[0]) > 0)
				return true;
		}
		return false;
	...
	public static boolean canSmelt(Player player, int barId) {
		...
		if (!hasOres(player, barId)) {
			...
			if (player.getOres()[0] > 0 && player.getOres()[1] > 0 && player.getOres()[1] != 453) {
				requirement = "To make " + anOrA(barId) + " " + ItemDefinition.forId(barId).getName()
						+ ", you need some " + ItemDefinition.forId(player.getOres()[0]).getName().replace(" ore", "")
						+ " and " + ItemDefinition.forId(player.getOres()[1]).getName() + ".";

			} else if (player.getOres()[0] > 0 && player.getOres()[1] == -1) {
				requirement = "To make " + anOrA(barId) + " " + ItemDefinition.forId(barId).getName()
						+ ", you need some " + ItemDefinition.forId(player.getOres()[0]).getName() + ".";
			} else if (player.getOres()[0] > 0 && player.getOres()[1] == 453) { // The bar uses custom coal amount
				requirement = "To make " + anOrA(barId) + " " + ItemDefinition.forId(barId).getName()
						+ ", you need some " + ItemDefinition.forId(player.getOres()[0]).getName().replace(" ore", "")
						+ " and " + getCoalAmount(barId) + " " + ItemDefinition.forId(player.getOres()[1]).getName()
						+ " ores.";
			}
			...
			case 7447:
				...
				if (bar > 0) {
					...
					player.getPacketSender().sendEnterAmountPrompt(
							"How many " + ItemDefinition.forId(bar).getName() + "s would you like to smelt?");
				}
				return true;
		}
		return false;
	...
	public static int getBarAmount(Item item) {
		String name = item.getDefinition().getName().toLowerCase();
		if (name.contains("scimitar") || name.contains("claws") || name.contains("crossbow")
				|| name.contains("longsword") || name.contains("sq shield") || name.contains("full helm")) {
			return 2;
		} else if (name.contains("2h sword") || name.contains("warhammer") || name.contains("battleaxe")
				|| name.contains("chainbody") || name.contains("platelegs") || name.contains("plateskirt")
