				...
				while (x > 0) {
					if (!hasRequirements(player, pouch))
						...
						if (pouch == Pouch.SPIRIT_DREADFOWL)
							AchievementsOLD.finishAchievement(player, AchievementDataOLD.INFUSE_A_DREADFOWL_POUCH);
						else if (pouch == Pouch.STEEL_TITAN) {
							AchievementsOLD.doProgress(player, AchievementDataOLD.INFUSE_25_TITAN_POUCHES);
							AchievementsOLD.doProgress(player, AchievementDataOLD.INFUSE_250_STEEL_TITAN_POUCHES);
						}
						x--;
					}
				}
