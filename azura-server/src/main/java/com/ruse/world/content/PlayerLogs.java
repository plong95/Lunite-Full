...
public class PlayerLogs {
	...
	public static void logSomething(String file, String name, String writable) {
		if (!GameSettings.LOG_DONATIONS)
			return;

		new Thread(() -> {
			try {
				Misc.createFilesIfNotExist(FILE_PATH + name + "/" + file + ".txt", false);
				FileWriter fw = new FileWriter(FILE_PATH + name + "/" + file + ".txt", true);
				fw.write(getTime() + writable + "\t");
				fw.write(System.lineSeparator());
				fw.close();

				Misc.createFilesIfNotExist(FILE_PATH + "all/" + file + ".txt", false);
				FileWriter all = new FileWriter(FILE_PATH +  "all/" + file + ".txt", true);
				all.write(getTime() + writable + "\t");
				all.write(System.lineSeparator());
				all.close();

				Misc.createFilesIfNotExist(WORLD_FILE_PATH +  name + ".txt", false);
				FileWriter world = new FileWriter(WORLD_FILE_PATH + name + ".txt", true);
				world.write(getTime() + " [ " + file + " ] " + writable + "\t");
				world.write(System.lineSeparator());
				world.close();
			} catch (Exception e) {
	...
	public static void logNews( String writable) {
		if (!GameSettings.LOG_DONATIONS)
			return;

		new Thread(() -> {
			try {
				Misc.createFilesIfNotExist(WORLD_FILE_PATH +  "news.txt", false);
				FileWriter world = new FileWriter(WORLD_FILE_PATH +  "news.txt", true);
	...
	public static void logStaff( String writable) {
		if (!GameSettings.LOG_DONATIONS)
			return;

		new Thread(() -> {
			try {
				Misc.createFilesIfNotExist(WORLD_FILE_PATH +  "staff.txt", false);
				FileWriter world = new FileWriter(WORLD_FILE_PATH +  "staff.txt", true);
				world.write(getTime() + writable + "\t");
				world.write(System.lineSeparator());
				world.close();
			} catch (Exception e) {
	...
	public static void npccoords(String file, int npcId, Position pos) {
		new Thread(() -> {
			try {
				...
				fw.write(System.lineSeparator());
				fw.write("		\"walking-policy\": {\"coordinate\": true,\"radius\": 1");
				fw.write(System.lineSeparator());
				fw.write("        }");
				fw.write(System.lineSeparator());
				fw.write("    },");
