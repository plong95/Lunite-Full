...
public class Construction {
	...
	public static void enterFriendsHouse(Player p, String p2) {
		...
		if (friend.getRegionInstance() == null
				|| !(friend.getRegionInstance().getType() == RegionInstanceType.CONSTRUCTION_HOUSE)
						&& !(friend.getRegionInstance().getType() == RegionInstanceType.CONSTRUCTION_DUNGEON)) {
	...
	public static void enterHouse(final Player me, final Player houseOwner, final boolean buildingMode) {
		if (me.getRegionInstance() == null
				|| !(me.getRegionInstance().getType() == RegionInstanceType.CONSTRUCTION_HOUSE)
						&& !(me.getRegionInstance().getType() == RegionInstanceType.CONSTRUCTION_DUNGEON)) {
			createPalette(me);
			return;
		}
			...
			public void execute() {
				...
				case 2:
					...
					if (me.getConstructionCoords() != null) {
						me.moveTo(new Position(me.getConstructionCoords()[0], me.getConstructionCoords()[1], 0));
			...
			for (int x = 0; x < 13; x++) {
				for (int y = 0; y < 13; y++) {
					...
					if (p.getHouseRooms()[z][x][y].getX() == 0)
						continue;
					PaletteTile tile = new PaletteTile(p.getHouseRooms()[z][x][y].getX(),
							p.getHouseRooms()[z][x][y].getY(), p.getHouseRooms()[z][x][y].getZ(),
		...
		for (int x = 0; x < 13; x++) {
			for (int y = 0; y < 13; y++) {
				...
				if (p.getHouseRooms()[4][x][y] == null) {
					tile = new PaletteTile(RoomData.DUNGEON_EMPTY.getX(),
							RoomData.DUNGEON_EMPTY.getY(), 0, 0);
				} else {
					tile = new PaletteTile(p.getHouseRooms()[4][x][y].getX(), p.getHouseRooms()[4][x][y].getY(),
							p.getHouseRooms()[4][x][y].getZ(), p.getHouseRooms()[4][x][y].getRotation());
				}
	...
	public static void placeNPCs(Player p) {
		...
		if (p.getHouseServant() > 0) {
			...
			Servant npc = (Servant) new NPC(p.getHouseServant(),
		...
		for (HouseFurniture pf : p.getHouseFurniture()) {
			...
			HotSpots hs = HotSpots.forHotSpotIdAndCoords(pf.getHotSpotId(), pf.getStandardXOff(), pf.getStandardYOff(),
					room);
	...
	public static void placeAllFurniture(Player p, int x, int y, int z) {
		...
		for (HouseFurniture pf : owner.getHouseFurniture()) {
			...
			doFurniturePlace(hs, f, hsses, getMyChunkFor(actualX, actualY), actualX, actualY, room.getRotation(), p,
					false, z);
		}
	}
	...
	public static void doFurniturePlace(HotSpots s, Furniture f, ArrayList<HotSpots> hsses, int[] myTiles, int actualX,
			int actualY, int roomRot, Player p, boolean placeBack, int height) {
		int portalId = -1;
		...
		if (s.getHotSpotId() == 92) {
			...
			if (s.getObjectId() == 15329 || s.getObjectId() == 15328) {
				p.getPacketSender()
						.sendObject_cons(actualX, actualY,
								s.getObjectId() == 15328 ? (placeBack ? 15328 : f.getFurnitureId())
										: (placeBack ? 15329 : f.getFurnitureId() + 1),
								s.getRotation(roomRot), 0, height);
				offsetX += ConstructionData.getXOffsetForObjectId(f.getFurnitureId(),
						s.getXOffset() + (s.getObjectId() == 15329 ? 1 : -1), s.getYOffset(), roomRot,
						s.getRotation(0));
				offsetY += ConstructionData.getYOffsetForObjectId(f.getFurnitureId(),
						s.getXOffset() + (s.getObjectId() == 15329 ? 1 : -1), s.getYOffset(), roomRot,
						s.getRotation(0));
				p.getPacketSender()
						.sendObject_cons(offsetX, offsetY,
								s.getObjectId() == 15329 ? (placeBack ? 15328 : f.getFurnitureId())
										: (placeBack ? 15329 : f.getFurnitureId() + 1),
								s.getRotation(roomRot), 0, height);
			...
			if (s.getObjectId() == 15326 || s.getObjectId() == 15327) {
				p.getPacketSender().sendObject_cons(actualX, actualY,
						s.getObjectId() == 15327 ? (placeBack ? 15327 : f.getFurnitureId() + 1)
								: (placeBack ? 15326 : f.getFurnitureId()),
						s.getRotation(roomRot), 0, height);
				offsetX += ConstructionData.getXOffsetForObjectId(f.getFurnitureId(),
				...
				p.getPacketSender().sendObject_cons(offsetX, offsetY,
						s.getObjectId() == 15326 ? (placeBack ? 15327 : f.getFurnitureId() + 1)
								: (placeBack ? 15326 : f.getFurnitureId()),
						s.getRotation(roomRot), 0, height);
			...
			p.getPacketSender().sendObject_cons(actualX, actualY, placeBack ? 15348 : cornerObject, 1, type, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 1, placeBack ? 15348 : leftObject, 1, type, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 2, placeBack ? 15348 : leftObject, 1, type, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 3, placeBack ? 15348 : cornerObject, 2, type,
					height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 3, placeBack ? 15348 : upperObject, 2, type,
					height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 3, placeBack ? 15348 : upperObject, 2, type,
					height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 3, placeBack ? 15348 : cornerObject, 3, type,
					height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 2, placeBack ? 15348 : rightObject, 3, type,
					height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 1, placeBack ? 15348 : rightObject, 3, type,
					height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY, placeBack ? 15348 : cornerObject, 0, type,
					height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY, placeBack ? 15348 : downObject, 0, type, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY, placeBack ? 15348 : downObject, 0, type, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 1, placeBack ? 15348 : middleObject, 0, type,
					height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 1, placeBack ? 15348 : middleObject, 0, type,
					height);
			if (veryMiddleObject != 0)
				p.getPacketSender().sendObject_cons(actualX + 1, actualY + 2, veryMiddleObject, 0, 10, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 2, placeBack ? 15348 : middleObject, 0, type,
					height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 2, placeBack ? 15348 : middleObject, 0, type,
					height);
			...
			p.getPacketSender().sendObject_cons(actualX + 1, actualY, placeBack ? 15352 : f.getFurnitureId(), 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY, placeBack ? 15352 : f.getFurnitureId(), 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY, placeBack ? 15352 : f.getFurnitureId(), 2, 2,
					height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 1, placeBack ? 15352 : f.getFurnitureId(), 2, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 2, placeBack ? 15352 : f.getFurnitureId() + 1, 2,
					0, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 3, placeBack ? 15352 : f.getFurnitureId(), 1, 2,
					height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 3, placeBack ? 15352 : f.getFurnitureId(), 1, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 3, placeBack ? 15352 : f.getFurnitureId(), 1, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 3, placeBack ? 15352 : f.getFurnitureId(), 0, 2,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 2, placeBack ? 15352 : f.getFurnitureId(), 0, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 1, placeBack ? 15352 : f.getFurnitureId(), 0, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY, placeBack ? 15352 : f.getFurnitureId(), 3, 2, height);

		} else if (s.getHotSpotId() == 78) {
			actualX = ConstructionData.BASE_X + (myTiles[0] * 8);
			actualY = ConstructionData.BASE_Y + (myTiles[1] * 8);
			// south walls
			p.getPacketSender().sendObject_cons(actualX, actualY, placeBack ? 15369 : f.getFurnitureId(), 3, 2, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY, placeBack ? 15369 : f.getFurnitureId(), 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY, placeBack ? 15369 : f.getFurnitureId(), 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY, placeBack ? 15369 : f.getFurnitureId(), 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 6, actualY, placeBack ? 15369 : f.getFurnitureId(), 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY, placeBack ? 15369 : f.getFurnitureId(), 2, 2,
					height);
			// north walls
			p.getPacketSender().sendObject_cons(actualX, actualY + 7, placeBack ? 15369 : f.getFurnitureId(), 0, 2,
					height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 7, placeBack ? 15369 : f.getFurnitureId(), 1, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 7, placeBack ? 15369 : f.getFurnitureId(), 1, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 7, placeBack ? 15369 : f.getFurnitureId(), 1, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 6, actualY + 7, placeBack ? 15369 : f.getFurnitureId(), 1, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 7, placeBack ? 15369 : f.getFurnitureId(), 1, 2,
					height);
			// left walls
			p.getPacketSender().sendObject_cons(actualX, actualY + 1, placeBack ? 15369 : f.getFurnitureId(), 0, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 2, placeBack ? 15369 : f.getFurnitureId(), 0, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 5, placeBack ? 15369 : f.getFurnitureId(), 0, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 6, placeBack ? 15369 : f.getFurnitureId(), 0, 0,
					height);
			// right walls
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 1, placeBack ? 15369 : f.getFurnitureId(), 2, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 2, placeBack ? 15369 : f.getFurnitureId(), 2, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 5, placeBack ? 15369 : f.getFurnitureId(), 2, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 6, placeBack ? 15369 : f.getFurnitureId(), 2, 0,
					height);
		} else if (s.getHotSpotId() == 77) {
			actualX = ConstructionData.BASE_X + (myTiles[0] * 8);
			actualY = ConstructionData.BASE_Y + (myTiles[1] * 8);
			// left down corner
			p.getPacketSender().sendObject_cons(actualX, actualY, placeBack ? 15372 : f.getFurnitureId() + 1, 3, 10,
					height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY, placeBack ? 15371 : f.getFurnitureId() + 2, 0, 10,
					height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY, placeBack ? 15370 : f.getFurnitureId(), 0, 10,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 1, placeBack ? 15371 : f.getFurnitureId() + 2, 1, 10,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 2, placeBack ? 15370 : f.getFurnitureId(), 3, 10,
					height);
			// right down corner
			p.getPacketSender().sendObject_cons(actualX + 7, actualY, placeBack ? 15372 : f.getFurnitureId() + 1, 2, 10,
					height);
			p.getPacketSender().sendObject_cons(actualX + 6, actualY, placeBack ? 15371 : f.getFurnitureId() + 2, 0, 10,
					height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY, placeBack ? 15370 : f.getFurnitureId(), 2, 10,
					height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 1, placeBack ? 15371 : f.getFurnitureId() + 2, 3,
					10, height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 2, placeBack ? 15370 : f.getFurnitureId(), 3, 10,
					height);
			// upper left corner
			p.getPacketSender().sendObject_cons(actualX, actualY + 7, placeBack ? 15372 : f.getFurnitureId() + 1, 0, 10,
					height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 7, placeBack ? 15371 : f.getFurnitureId() + 2, 0,
					10, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 7, placeBack ? 15370 : f.getFurnitureId(), 0, 10,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 6, placeBack ? 15371 : f.getFurnitureId() + 2, 1, 10,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 5, placeBack ? 15370 : f.getFurnitureId(), 1, 10,
					height);
			// upper right corner
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 7, placeBack ? 15372 : f.getFurnitureId() + 1, 1,
					10, height);
			p.getPacketSender().sendObject_cons(actualX + 6, actualY + 7, placeBack ? 15371 : f.getFurnitureId() + 2, 0,
					10, height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 7, placeBack ? 15370 : f.getFurnitureId(), 2, 10,
					height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 6, placeBack ? 15371 : f.getFurnitureId() + 2, 3,
					10, height);
			p.getPacketSender().sendObject_cons(actualX + 7, actualY + 5, placeBack ? 15370 : f.getFurnitureId(), 1, 10,
					height);
			...
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 3, placeBack ? 15292 : combatringFloorsInner, 0,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 3, placeBack ? 15292 : combatringFloorsInner, 0,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 2, placeBack ? 15292 : combatringFloorsInner, 0,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 2, placeBack ? 15292 : combatringFloorsInner, 0,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 1, placeBack ? 15291 : combatringFloorsOuter, 3,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 1, placeBack ? 15291 : combatringFloorsOuter, 3,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 4, placeBack ? 15291 : combatringFloorsOuter, 1,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 4, placeBack ? 15291 : combatringFloorsOuter, 1,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 4, actualY + 3, placeBack ? 15291 : combatringFloorsOuter, 2,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 4, actualY + 2, placeBack ? 15291 : combatringFloorsOuter, 2,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 3, placeBack ? 15291 : combatringFloorsOuter, 0,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 2, placeBack ? 15291 : combatringFloorsOuter, 0,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 4, actualY + 1, placeBack ? 15289 : combatringFloorsCorner, 3,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 4, actualY + 4, placeBack ? 15289 : combatringFloorsCorner, 2,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 4, placeBack ? 15289 : combatringFloorsCorner, 1,
					22, height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 1, placeBack ? 15289 : combatringFloorsCorner, 0,
					22, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 4, placeBack ? 15277 : combatringStrings, 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 1, placeBack ? 15277 : combatringStrings, 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 4, placeBack ? 15277 : combatringStrings, 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 1, placeBack ? 15277 : combatringStrings, 0, 3,
					height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY, placeBack ? 15277 : combatringStrings, 1, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY, placeBack ? 15277 : combatringStrings, 1, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY, placeBack ? 15277 : combatringStrings, 1, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 4, actualY, placeBack ? 15277 : combatringStrings, 1, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY, placeBack ? 15277 : combatringStrings, 0, 3,
					height);
			p.getPacketSender().sendObject_cons(actualX + 1, actualY + 5, placeBack ? 15277 : combatringStrings, 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 2, actualY + 5, placeBack ? 15277 : combatringStrings, 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 3, actualY + 5, placeBack ? 15277 : combatringStrings, 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 4, actualY + 5, placeBack ? 15277 : combatringStrings, 3, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 5, placeBack ? 15277 : combatringStrings, 3, 3,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 5, placeBack ? 15277 : combatringStrings, 2, 3,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY, placeBack ? 15277 : combatringStrings, 1, 3, height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 4, placeBack ? 15277 : combatringStrings, 2, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 3, placeBack ? 15277 : combatringStrings, 2, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 2, placeBack ? 15277 : combatringStrings, 2, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX, actualY + 1, placeBack ? 15277 : combatringStrings, 2, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 4, placeBack ? 15277 : combatringStrings, 0, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 3, placeBack ? 15277 : combatringStrings, 0, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 2, placeBack ? 15277 : combatringStrings, 0, 0,
					height);
			p.getPacketSender().sendObject_cons(actualX + 5, actualY + 1, placeBack ? 15277 : combatringStrings, 0, 0,
					height);

			if (f.getFurnitureId() == 13145) {
				p.getPacketSender().sendObject_cons(actualX + 1, actualY + 1, placeBack ? 6951 : 13145, 0, 0, height);
				p.getPacketSender().sendObject_cons(actualX + 2, actualY + 1, placeBack ? 6951 : 13145, 0, 0, height);
				p.getPacketSender().sendObject_cons(actualX + 1, actualY, placeBack ? 6951 : 13145, 1, 0, height);
				p.getPacketSender().sendObject_cons(actualX + 1, actualY + 2, placeBack ? 6951 : 13145, 3, 0, height);
				if (!placeBack)
					p.getPacketSender().sendObject_cons(actualX + 1, actualY + 1, 13147, 0, 22, height);

				p.getPacketSender().sendObject_cons(actualX + 3, actualY + 3, placeBack ? 6951 : 13145, 0, 0, height);
				p.getPacketSender().sendObject_cons(actualX + 4, actualY + 3, placeBack ? 6951 : 13145, 0, 0, height);
				p.getPacketSender().sendObject_cons(actualX + 3, actualY + 2, placeBack ? 6951 : 13145, 1, 0, height);
				p.getPacketSender().sendObject_cons(actualX + 3, actualY + 4, placeBack ? 6951 : 13145, 3, 0, height);
				if (!placeBack)
			...
			for (int x = 0; x < s.getCarpetDim().getWidth() + 1; x++) {
				for (int y = 0; y < s.getCarpetDim().getHeight() + 1; y++) {
					boolean isEdge = (x == 0 && y == 0 || x == 0 && y == s.getCarpetDim().getHeight()
							|| y == 0 && x == s.getCarpetDim().getWidth()
							|| x == s.getCarpetDim().getWidth() && y == s.getCarpetDim().getHeight());
					boolean isWall = ((x == 0 || x == s.getCarpetDim().getWidth())
							&& (y != 0 && y != s.getCarpetDim().getHeight())
							|| (y == 0 || y == s.getCarpetDim().getHeight())
					...
					int offsetX = ConstructionData.BASE_X + (myTiles[0] * 8);
					int offsetY = ConstructionData.BASE_Y + (myTiles[1] * 8);
					offsetX += ConstructionData.getXOffsetForObjectId(f.getFurnitureId(), s.getXOffset() + x - 1,
							s.getYOffset() + y - 1, roomRot, s.getRotation(roomRot));
					offsetY += ConstructionData.getYOffsetForObjectId(f.getFurnitureId(), s.getXOffset() + x - 1,
							s.getYOffset() + y - 1, roomRot, s.getRotation(roomRot));
					if (isEdge)
						p.getPacketSender().sendObject_cons(offsetX, offsetY,
								placeBack ? s.getObjectId() + 2 : f.getFurnitureId(),
								HotSpots.getRotation_2(rot, roomRot), 22, height);
					else if (isWall)
						p.getPacketSender().sendObject_cons(offsetX, offsetY,
								placeBack ? s.getObjectId() + 1 : f.getFurnitureId() + 1,
								HotSpots.getRotation_2(rot, roomRot), s.getObjectType(), height);
					else
						p.getPacketSender().sendObject_cons(offsetX, offsetY,
								placeBack ? s.getObjectId() : f.getFurnitureId() + 2,
								HotSpots.getRotation_2(rot, roomRot), s.getObjectType(), height);
				}
			}
		} else if (s.isMutiple()) {

			Room room = p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4
					: p.getPosition().getZ()][myTiles[0] - 1][myTiles[1] - 1];
			for (HotSpots find : hsses) {
				...
				actualX1 += ConstructionData.getXOffsetForObjectId(find.getObjectId(), find, roomRot);
				int actualY1 = ConstructionData.BASE_Y + (myTiles[1] * 8);
				actualY1 += ConstructionData.getYOffsetForObjectId(find.getObjectId(), find, roomRot);

				p.getPacketSender().sendObject_cons(actualX1, actualY1,
						placeBack ? s.getObjectId() : f.getFurnitureId(), find.getRotation(roomRot),
	...
	public static void handleFifthObjectClick(int obX, int obY, int objectId, Player p) {
		switch (objectId) {
		case 13497:
		case 13499:
		case 13501:
	...
	public static void handleFourthObjectClick(int obX, int obY, int objectId, Player p) {
	...
	public static boolean handleButtonClick(int buttonId, Player p) {
		...
		case 2494:
			...
			if (p.getDialogueActionId() == 425 || p.getDialogueActionId() == 426 || p.getDialogueActionId() == 427
					|| p.getDialogueActionId() == 429 || p.getDialogueActionId() == 431) {
				p.getInventory().add(7688, 1);
				return true;
			}
		...
		case 2495:
			...
			if (p.getDialogueActionId() == 429 || p.getDialogueActionId() == 431 || p.getDialogueActionId() == 429) {
			...
			if (p.getDialogueActionId() == 452) {
				if (p.getRegionInstance().getOwner() == p) {
					House house = p.getRegionInstance().getType() == RegionInstanceType.CONSTRUCTION_HOUSE
							? (House) p.getRegionInstance()
							: ((HouseDungeon) p.getRegionInstance()).getHouse();
		...
		case 2461:
			...
			if (p.getDialogueActionId() == 438) {
				...
				if (room != null) {
					p.getPacketSender()
							.sendMessage("You did something retarded and now there is a under you for some reason.");
					p.getPacketSender().sendInterfaceRemoval();
				} else {
			...
			if (p.getDialogueActionId() == 416) {
				...
				if (room.getType() != ConstructionData.EMPTY) {
					p.getPacketSender().sendMessage(
							"You did something retarded and now there is a under above you for some reason.");
					p.getPacketSender().sendInterfaceRemoval();
				} else {
					createRoom(room_1.getType() == ConstructionData.SKILL_HALL_DOWN ? ConstructionData.SKILL_ROOM
			...
			if (p.getDialogueActionId() == 414) {
				...
				if (room != null) {
					p.getPacketSender().sendMessage(
							"You did something retarded and now there is a room above you for some reason.");
					p.getPacketSender().sendInterfaceRemoval();
				} else {
					createRoom(room_1.getType() == ConstructionData.SKILL_ROOM ? ConstructionData.SKILL_HALL_DOWN
		...
		case 2462:// no option
			...
			if (p.getDialogueActionId() == 414 || p.getDialogueActionId() == 416 || p.getDialogueActionId() == 419
					|| p.getDialogueActionId() == 457) {
				p.getPacketSender().sendInterfaceRemoval();
				return true;
			}
			break;
	...
	public static boolean roomExists(Player p) {
		...
		Room room = p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4
				: p.getPosition().getZ()][myTiles[0] - 1 + xOff][myTiles[1] - 1 + yOff];
		if (room == null)
			return false;
		if (room.getType() == ConstructionData.BUILDABLE || room.getType() == ConstructionData.EMPTY
				|| room.getType() == ConstructionData.DUNGEON_EMPTY)
			return false;
		return true;
	}

	public static boolean handleRemoveClick(int obX, int obY, int objectId, Player p) {
		...
		int[] myTiles = getMyChunk(p);
		int roomRot = p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4
				: p.getPosition().getZ()][myTiles[0] - 1][myTiles[1] - 1].getRotation();
		Room room = p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4
				: p.getPosition().getZ()][myTiles[0] - 1][myTiles[1] - 1];
		ArrayList<HotSpots> hsses = HotSpots.forObjectId_3(f.getHotSpotId());
		if (hsses.isEmpty())
		...
		if (hsses.size() == 1)
			...
			for (HotSpots find : hsses) {
				...
				actualX += ConstructionData.getXOffsetForObjectId(find.getObjectId(), find, roomRot);
				int actualY = ConstructionData.BASE_Y + (myTiles[1] * 8);
				actualY += ConstructionData.getYOffsetForObjectId(find.getObjectId(), find, roomRot);
		...
		if (objectId == 15270 || objectId == 15273 || objectId == 15274 || objectId >= 13588 && objectId <= 13597) {
			...
			if (room.getType() == ConstructionData.SKILL_ROOM || room.getType() == ConstructionData.SKILL_HALL_DOWN
		...
		doFurniturePlace(hs, f, hsses, myTiles, obX, obY, roomRot, p, true, p.getPosition().getZ());
	...
	public static boolean handleSpaceClick(int obX, int obY, int objectId, Player p) {

		int[] myTiles = getMyChunk(p);
		int roomRot = p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4
				: p.getPosition().getZ()][myTiles[0] - 1][myTiles[1] - 1].getRotation();

		ArrayList<HotSpots> hsses = HotSpots.forObjectId_2(objectId);
		if (hsses.isEmpty())
			return false;
		...
		HotSpots hs = null;
		int myRoom = p.getHouseRooms()[p.inConstructionDungeon() ? 4 : p.getPosition().getZ()][myTiles[0]
				- 1][myTiles[1] - 1].getType();
		if (hsses.size() == 1)
			hs = hsses.get(0);
		else {
			for (HotSpots find : hsses) {
				...
				int actualY = ConstructionData.BASE_Y + (myTiles[1] * 8);
				actualY += ConstructionData.getYOffsetForObjectId(find.getObjectId(), find, roomRot);
				if (p.getBuildFurnitureX() == actualX && p.getBuildFurnitureY() == actualY
		...
		if (hs == null)
			return true;
		ArrayList<Furniture> f = Furniture.getForHotSpotId(hs.getHotSpotId());
		if (f == null)
			return false;
	...
	public static void createRoom(int roomType, Player p, int toHeight) {
		RoomData rd = RoomData.forID(roomType);
		if (rd == null)
			return;
		...
		if (yOnTile == 7)
			direction = UP;
		int rotation = RoomData.getFirstElegibleRotation(rd, direction);
		if (toHeight == 100) {
			...
			doFurniturePlace(HotSpots.SKILL_HALL_STAIRS_1, Furniture.forFurnitureId(stairId), null, myTiles,
					ConstructionData.BASE_X + (myTiles[0] * 8) + 3, ConstructionData.BASE_Y + (myTiles[1] * 8) + 3,
					rotation, p, false, 1);
		...
		if (toHeight == 101) {
			direction = SAME;
			toHeight = 0;
			rotation = p.getRegionInstance().getOwner().getHouseRooms()[1][myTiles[0] - 1][myTiles[1] - 1]
					.getRotation();
			int stairId = 0;
			for (HouseFurniture furn : p.getHouseFurniture()) {
			...
			doFurniturePlace(HotSpots.SKILL_HALL_STAIRS, Furniture.forFurnitureId(stairId), null, myTiles,
					ConstructionData.BASE_X + (myTiles[0] * 8) + 3, ConstructionData.BASE_Y + (myTiles[1] * 8) + 3,
					rotation, p, false, 1);
		...
		if (toHeight == 1) {
			Room r = p.getRegionInstance().getOwner().getHouseRooms()[0][(myTiles[0] - 1) + xOff][(myTiles[1] - 1)
					+ yOff];
			if (r.getType() == ConstructionData.EMPTY || r.getType() == ConstructionData.BUILDABLE
					|| r.getType() == ConstructionData.GARDEN || r.getType() == ConstructionData.FORMAL_GARDEN) {
		...
		house.getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4 : toHeight][(myTiles[0] - 1)
				+ xOff][(myTiles[1] - 1) + yOff] = new Room(rotation, roomType, 0);
		 ...
		 * house.getSecondaryPalette()); } else {
	...
	public static void rotateRoom(int wise, Player p) {
		...
		Room r = p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4
				: p.getPosition().getZ()][chunkX][chunkY];
		...
		p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4
				: p.getPosition().getZ()][chunkX][chunkY].setRotation(toRot);
		p.setConstructionCoords(new int[] { p.getPosition().getX(), p.getPosition().getY() });
		p.getRegionInstance().destruct();
		createPalette(p);
		/*
		 ...
		 * house.getSecondaryPalette()); } else {
		 * p.getPacketSender().constructMapRegionForConstruction(house.getPalette()); }
		 * placeAllFurniture(p, chunkX, chunkY, p.getFields().inDungeon() ? 4 :
		 * p.getPosition().getZ()); p.getPacketSender().sendInterfaceRemoval();
		 */
	...
	public static boolean handleFirstObjectClick(int obX, int obY, int objectId, Player p) {
		...
		/*
		 * if (ConstructionActions.handleFirstObjectClick(p, objectId)) return true;
		 */
		// Stairs
		if (objectId == Furniture.OAK_STAIRCASE.getFurnitureId()
				|| objectId == Furniture.OAK_STAIRCASE_1.getFurnitureId()
				|| objectId == Furniture.TEAK_STAIRCASE.getFurnitureId()
				|| objectId == Furniture.TEAK_STAIRCASE_1.getFurnitureId()
				|| objectId == Furniture.MARBLE_STAIRCASE.getFurnitureId()
				|| objectId == Furniture.MARBLE_STAIRCASE_1.getFurnitureId()) {
			int myTiles[] = getMyChunk(p);
			Room room = p.getRegionInstance().getOwner().getHouseRooms()[1][myTiles[0] - 1][myTiles[1] - 1];
			if (room == null) {
				...
				p.setConstructionCoords(
						new int[] { p.getPosition().getX(), p.getPosition().getY(), p.getPosition().getZ() + 1 });
				p.moveTo(
						new Position(ConstructionData.MIDDLE_X, ConstructionData.MIDDLE_Y, p.getPosition().getZ() + 1));
				if (p.inConstructionDungeon()) {
				...
				p.moveTo(new Position(p.getConstructionCoords()[0], p.getConstructionCoords()[1],
						p.getConstructionCoords()[2]));
				p.setConstructionCoords(null);
				p.getPacketSender().sendInterfaceRemoval();
				return true;
			}
		}
		if (objectId == Furniture.DUNGEON_ENTRANCE.getFurnitureId()) {
			p.getPacketSender().sendMessage("This has been disabled for now.");
			return true;
			/*
			 * int myTiles[] = getMyChunk(p); Room room =
			 * p.getRegionInstance().getOwner().getHouseRooms()[4][myTiles[0] -
			 * 1][myTiles[1] - 1]; if (room == null) { DialogueManager.start(p,
			 * ConstructionDialogues.buildUpstairs(p, 419)); return true; } else {
			 * p.setInConstructionDungeon(true); p.setConstructionCoords(new int[] {
			 * p.getPosition().getX(), p.getPosition().getY(), 4 }); p.moveTo(new
			 * Position(ConstructionData.MIDDLE_X, ConstructionData.MIDDLE_Y, 4)); if
			 * (p.inConstructionDungeon()) { p.getPacketSender().constructMapRegion(((House)
			 * p.getRegionInstance()).getSecondaryPalette()); placeAllFurniture(p, 4);
			 * p.getPacketSender().constructMapRegion(((House)
			 * p.getRegionInstance()).getSecondaryPalette()); } else {
			 * p.getPacketSender().constructMapRegion(((House)
			 * p.getRegionInstance()).getPalette()); placeAllFurniture(p, 0);
			 * placeAllFurniture(p, 1); p.getPacketSender().constructMapRegion(((House)
			 * p.getRegionInstance()).getPalette()); } p.moveTo(new
			 * Position(p.getConstructionCoords()[0], p.getConstructionCoords()[1],
			 * p.getConstructionCoords()[2])); p.setConstructionCoords(null); return true; }
			 */
		}
		return false;
	}
	...
	public static void deleteRoom(Player p, int toHeight) {
		...
		int roomType = p.inConstructionDungeon() ? ConstructionData.DUNGEON_EMPTY : ConstructionData.EMPTY;
			...
			for (int z = 0; z < p.getRegionInstance().getOwner().getHouseRooms().length; z++) {
				for (int x = 0; x < p.getRegionInstance().getOwner().getHouseRooms()[z].length; x++) {
					for (int y = 0; y < p.getRegionInstance().getOwner().getHouseRooms()[z][x].length; y++) {
						...
						if (r1 == null)
							continue;
						if (r1.getType() == ConstructionData.GARDEN || r1.getType() == ConstructionData.FORMAL_GARDEN) {
							gardenAmt++;
						}
					}
				}
		...
		if (p.getPosition().getZ() == 0) {
			...
			p.getRegionInstance().getOwner().getHouseRooms()[p.inConstructionDungeon() ? 4
					: toHeight][chunkX][chunkY] = new Room(0, roomType, 0);
		} else {
			if (p.inConstructionDungeon()) {
	...
	public static void handleInterfaceCrosses(ArrayList<Furniture> items, Player c, HotSpots hs) {
		int i = 1000;
		...
		for (Furniture f : items) {
			...
			c.getPacketSender().sendString(38280 + (i - 1000) * 6, "Level: " + f.getLevel());
			int i2 = 0;
			boolean canMake = true;
			for (int i1 = 0; i1 < f.getRequiredItems().length; i1++) {
				c.getPacketSender().sendString((38276 + i2) + (i - 1000) * 6, f.getRequiredItems()[i1][1] + " x "
			...
			if (f.getFurnitureRequired() != -1) {
				...
				if (canMake) {
					...
					for (HouseFurniture pf : c.getHouseFurniture()) {
						if (pf.getRoomX() == myTiles[0] - 1 && pf.getRoomY() == myTiles[1] - 1) {
							if (pf.getHotSpot(
									c.getHouseRooms()[c.inConstructionDungeon() ? 4 : c.getPosition().getZ()][myTiles[0]
											- 1][myTiles[1] - 1].getRotation()) == hs) {
								if (pf.getFurnitureId() != fur.getFurnitureId()) {
									canMake = false;
								} else {
	...
	public static void handleItemClick(int itemID, Player p) {
		...
		ArrayList<HotSpots> hsses = HotSpots.forObjectId_2(p.getBuildFurnitureId());
		if (hsses.isEmpty())
			return;
		int[] myTiles = getMyChunk(p);
		int toHeight = (p.getRegionInstance().getType() == RegionInstanceType.CONSTRUCTION_DUNGEON ? 4
				: p.getPosition().getZ());
		int roomRot = p.getRegionInstance().getOwner().getHouseRooms()[toHeight][myTiles[0] - 1][myTiles[1] - 1]
				.getRotation();
		int myRoomType = p.getRegionInstance().getOwner().getHouseRooms()[toHeight][myTiles[0] - 1][myTiles[1] - 1]
				.getType();
		HotSpots s = null;
		if (hsses.size() == 1) {
			s = hsses.get(0);
		...
		if (!buildActions(p, f, s))
			return;
		int actualX = ConstructionData.BASE_X + (myTiles[0] * 8);
		actualX += ConstructionData.getXOffsetForObjectId(f.getFurnitureId(), s,
				p.getRegionInstance().getOwner().getHouseRooms()[toHeight][myTiles[0] - 1][myTiles[1] - 1]
						.getRotation());
		int actualY = ConstructionData.BASE_Y + (myTiles[1] * 8);
		actualY += ConstructionData.getYOffsetForObjectId(f.getFurnitureId(), s, roomRot);
		if (s.getRoomType() != myRoomType && s.getCarpetDim() == null) {
			p.getPacketSender().sendMessage("You can't build this furniture in this room.");
			return;
		}
		doFurniturePlace(s, f, hsses, myTiles, actualX, actualY, roomRot, p, false, p.getPosition().getZ());
		HouseFurniture pf = new HouseFurniture(myTiles[0] - 1, myTiles[1] - 1, toHeight, s.getHotSpotId(),
	...
	public static int[] getConvertedCoords(int tileX, int tileY, int[] myTiles, Room room) {
		int actualX = ConstructionData.BASE_X + (myTiles[0] * 8);
		actualX += ConstructionData.getXOffsetForObjectId(1, tileX, tileY, room.getRotation(), 0);
		int actualY = ConstructionData.BASE_Y + (myTiles[1] * 8);
		actualY += ConstructionData.getYOffsetForObjectId(1, tileX, tileY, room.getRotation(), 0);
	...
	public static boolean buildingHouse(Player player) {
		return player.getRegionInstance() != null
