...
public class DialogueManager {
	...
	public static void editOptions(int dialogueId, int optionId, String newOption) {
		Dialogue oldDialogue = dialogues.get(dialogueId);
		Dialogue newDialogue = new Dialogue() {
			...
			@Override
			public DialogueType type() {
				return oldDialogue.type();
			}

			@Override
			public DialogueExpression animation() {
				return oldDialogue.animation();
			}

			@Override
			public String[] dialogue() {
				String[] newDialogueLines = oldDialogue.dialogue();
				if (newDialogueLines.length >= optionId)
					newDialogueLines[optionId - 1] = newOption;
				return newDialogueLines;
			}

			@Override
			public int nextDialogueId() {
				return oldDialogue.nextDialogueId();
			}

			@Override
			public int npcId() {
				return oldDialogue.npcId();
			}
		...
		dialogues.put(dialogueId, newDialogue);
	}
	...
	private static void showDialogue(Player player, Dialogue dialogue) {
		if (dialogue.id() == 20010 && player.getCurseOfArrav().getQuestProgress() == 1){
			player.getInventory().add(15112, 1);
		}
		if (dialogue.id() == 20012 && player.getCurseOfArrav().getQuestProgress() == 1){
			player.getInventory().add(3430, 1);
		}
		...
		case ITEM_STATEMENT:
			...
			break;
			case STATEMENT:
				startDialogueChildId = STATEMENT_DIALOGUE_ID[lines.length - 1] + 1;
				for (int i = 0; i < lines.length; i++) {
					player.getPacketSender().sendString(startDialogueChildId++, lines[i]);
				}
				player.getPacketSender().sendString(startDialogueChildId++, "Click here to continue");
		...
		case OPTION:
			...
			for (int i = 0; i < lines.length; i++) {
				if (lines[i].equalsIgnoreCase("[Dissolve]") ){
					player.getPacketSender().sendString(firstChildId + i, "Dissolve all dissolveable items for "
							+ Misc.insertCommasToNumber(player.getDissolving().getTotalOrbAmount(player.getDissolving().getDissolvableItemsInInv()))
							+ " Tokens");
				} else if (lines[i].equalsIgnoreCase("[Dissolve1]") ){
					player.getPacketSender().sendString(firstChildId + i, "Dissolve item for "
							+ Misc.insertCommasToNumber(player.getDissolving().getTotalOrbAmount(player.getAttemptDissolve()))
							+ " Tokens");
				} else if (lines[i].equalsIgnoreCase("[petupgrading]") &&
	...
	public static void sendItemStatement(Player player, int itemId, int zoom, String statement) {
		int startDialogueChildId = 4885;
		int headChildId = startDialogueChildId - 2;
		player.getPacketSender().sendInterfaceModel(headChildId, itemId, zoom);
		player.getPacketSender().sendInterfaceAnimation(headChildId, new Animation(-1));

		player.getPacketSender().sendString(startDialogueChildId - 1, "");
		player.getPacketSender().sendString(startDialogueChildId + 0, statement);
