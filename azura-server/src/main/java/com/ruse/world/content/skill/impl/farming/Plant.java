...
public class Plant {
	...
	public void harvest(final Player player) {
		if (harvesting)
			return;
		final boolean magicSecateurs = player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 7409
				|| player.getInventory().contains(7409);
		...
		if (magicSecateurs || player.getInventory().contains(harvestId)) {
			...
			TaskManager.submit(new Task(1, player, true) {
				...
				public void execute() {
					...
					Achievements.doProgress(player, Achievements.Achievement.FARMING);

					AchievementsOLD.finishAchievement(player, AchievementDataOLD.HARVEST_A_CROP);
					...
					if (id == 219) {
						AchievementsOLD.doProgress(player, AchievementDataOLD.HARVEST_10_TORSTOLS);
						AchievementsOLD.doProgress(player, AchievementDataOLD.HARVEST_1000_TORSTOLS);
					}
					harvested++;
			...
			player.getPacketSender()
					.sendMessage("You need " + Misc.anOrA(name) + " " + name + " to harvest these plants.");
		}
	}
			...
			for (int i = 0; i < elapsed / grow; i++) {
				...
				if (isWatered()) {
					...
					if (stage >= Plants.values()[plant].stages) {
						player.getPacketSender()
								.sendMessage("<img=5> <shad=996699>A seed which you planted has finished growing!");
						return;
					}
				}
