...
public class DarkSupremeCombatScript implements CombatStrategy {
	...
	public boolean customContainerAttack(Character entity, Character victim) {
		...
		if (Misc.getRandom(100) <= 25 && !supreme.isChargingAttack()) {
			...
			supreme.setChargingAttack(true);
			for (int i = 0; i < 50; i++) {
				int randX = Misc.random(3032, 3047), randY = Misc.random(4511, 4521);
					...
					public void execute() {
						TaskManager.submit(new Task(2, supreme, false) {
							...
							public void execute() {
								...
								for (Player players : supreme.getClosePlayers(32)) {
									if (players.getPosition().getX() == p.getX() &&
											players.getPosition().getY() == p.getY() &&
											players.getPosition().getZ() == supreme.getPosition().getZ()) {
										int damage = Misc.exclusiveRandom(300, 800);
										player.dealDamage(new Hit(damage, Hitmask.RED, CombatIcon.NONE));
										players.sendMessage("@blu@You were damaged by rocks!");
										players.performGraphic(new Graphic(2149));
									}
								}
								supreme.setChargingAttack(false);
								this.stop();
