...
public class CombatDistanceSession {
	...
	public void process() {
		...
		int realDistance = builder.getStrategy().getCombatType() == CombatType.MAGIC ? builder.getStrategy().attackDistance(builder.getCharacter())+1 : builder.getStrategy().attackDistance(builder.getCharacter());
		if (Locations.goodDistance(builder.getCharacter(), victim,
				realDistance)) {
			sucessFul();
			stop();
			return;
