	...
	public static Dialogue get(Player p, int stage) {
		...
		case -2:
			dialogue = new Dialogue() {
				...
				public String[] dialogue() {
					return new String[] { "Great! Now you need to choose a difficulty.",
							"This is a multiplier applied to all EXP you gain.",
							"It can make the game easier, or harder.",
							"Be sure though, as difficulties can't be changed." };
				};
		...
		case -1:
			dialogue = new Dialogue() {
				...
				@Override
				public String[] dialogue() {
					return new String[] { "Easy (25% faster)", "Regular (base)", "Hard (25% slower)",
							"Extreme (50% slower)", "More info..." };
				};
		...
		case 0:
			dialogue = new Dialogue() {
				...
				@Override
				public String[] dialogue() {
					return new String[] { "You're " + Misc.anOrA(p.getGameMode().toString()) + " "
							+ Misc.capitalizeString(Misc.formatText(p.getGameMode().toString())) + " mode character. ",
							"You're playing on the "
									+ Misc.capitalizeString(Misc.formatText(p.getDifficulty().toString()))
									+ " difficulty!",
			...
			dialogue = new Dialogue() {
				...
				public String[] dialogue() {
					return new String[] { "You can earn money doing many different things in",
							GameSettings.RSPS_NAME + ". For example, see those Thieving stalls infront of ",
							"you? You can steal items from them and sell them to the",
							"merchant who is standing over there." };
				};
			...
			dialogue = new Dialogue() {
				...
				public String[] dialogue() {
					return new String[] { "Enough of the boring stuff, let's show you some creatures!",
							"There are a bunch of bosses to fight in " + GameSettings.RSPS_NAME + ".",
		...
		case 8:
			dialogue = new Dialogue() {
				...
				@Override
				public String[] dialogue() {
					return new String[] { GameSettings.RSPS_NAME + " also has a lot of enjoyable minigames.",
		...
		case 9:
			dialogue = new Dialogue() {
				...
				@Override
				public String[] dialogue() {
					return new String[] { "This is the member's zone.",
							"Players who have a Member rank can teleport here",
		...
		case 10:
			dialogue = new Dialogue() {
				...
				@Override
				public String[] dialogue() {
					return new String[] { "To receive a member rank, you need to claim",
							"a member scroll, which is a one-time purchase.", "Scrolls can be purchased from the store",
		...
		case 11:
			dialogue = new Dialogue() {
				...
				public String[] dialogue() {
					return new String[] {
							GameSettings.RSPS_NAME + " is a competitive game. Next to you is a scoreboard",
		...
		case 12:
			dialogue = new Dialogue() {
				...
				@Override
				public void specialAction() {
					p.moveTo(GameSettings.STARTER.copy());
		...
		case 13:
			dialogue = new Dialogue() {
				...
				@Override
				public String[] dialogue() {
					return new String[] { "If you have any more questions, ask in the clan chat.",
							"You may also wish to check the ::forums and ::wikia",
							" Have fun playing " + GameSettings.RSPS_NAME + "!" };// "If you have any more questions,
																					// simply use the ::help", "command
																					// and a staff member should get
																					// back to you.", "You can also join
																					// the clanchat channel 'help' and
																					// ask", "other players for help
																					// there too. Have fun playing
																					// "+GameSettings.RSPS_NAME+"!"};
				};
			...
			dialogue = new Dialogue() {
				...
				public void specialAction() {
					...
					if (ConnectionHandler.getStarters(p.getHostAddress()) <= GameSettings.MAX_STARTERS_PER_IP
							&& p.getGameMode() == GameMode.NORMAL) {
						//p.getInventory().add(5021, 5).add(1153, 1).add(1115, 1).add(1067, 1).add(882, 1).add(1351, 1)
						//		.add(1265, 1).add(1007, 1).add(362, 50).add(3904, 1).add(757, 1).add(290, 1);
						...
						if (p.getGameMode() == GameMode.NORMAL) {
							//p.getInventory().add(995, 1).add(1153, 1).add(1115, 1).add(1067, 1).add(882, 1).add(1351, 1)
							//		.add(1265, 1).add(1007, 1).add(362, 50).add(3904, 1).add(757, 1).add(290, 1);
							p.setReceivedStarter(true);
							p.getPacketSender().sendMessage("You claim a normal starter kit, but without valuables...");
							p.getPacketSender().sendMessage("This is because you've created more than "
									+ GameSettings.MAX_STARTERS_PER_IP + " accounts.");
						}
						if (p.getGameMode() == GameMode.IRONMAN) {
							p.getInventory().add(1351, 1).add(1265, 1).add(16691, 1).add(9704, 1).add(17239, 1)
									.add(16669, 1).add(16339, 1).add(6068, 1).add(9703, 1).add(3904, 1).add(757, 1)
									.add(290, 1);
							p.setReceivedStarter(true);
							p.getPacketSender().sendMessage("You claim your Ironman starter kit!");
						}
						if (p.getGameMode() == GameMode.ULTIMATE_IRONMAN) {
							p.getInventory().add(1351, 1).add(1265, 1).add(3904, 1).add(757, 1).add(17239, 1)
									.add(16669, 1).add(16339, 1).add(290, 1);
							p.setReceivedStarter(true);
							p.getPacketSender().sendMessage("You claim your Ultimate Ironman starter kit!");
						}
					}
					World.sendMessage("<shad=1><col=F5FF3B><img=5> " + p.getUsername()
						...
						protected void execute() {
							if (p != null && p.isRegistered()) {
								p.getPacketSender().sendMessage(
										"<img=5> @blu@The home area includes shops and skills for you to start with.");
								if (p.getGameMode() == GameMode.ULTIMATE_IRONMAN)
									p.getPacketSender().sendMessage("<img=5> @gre@" + p.getUsername()
