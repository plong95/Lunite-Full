...
public enum CombatSpecial {
	DRAGON_CLAWS_U(new int[] { 23888 }, 30, 1, 1, CombatType.MELEE, WeaponInterface.CLAWS) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(10961));
			player.performGraphic(new Graphic(3001));
			return new CombatContainer(player, target, 4, CombatType.MELEE, true);
		}
	},

	DRAGON_CLAWS(new int[] { 23696 }, 40, 1, 1, CombatType.MELEE, WeaponInterface.CLAWS) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(10961));
			player.performGraphic(new Graphic(3001));
	...
	TORNADO_KATANA(new int[] { 14018 }, 30, 1, 1, CombatType.MELEE, WeaponInterface.SCIMITAR) {
		@Override
		public CombatContainer container(Player player, Character target) {
	...
	ST_PAT(new int[] {  13801 }, 30, 1, 1, CombatType.MELEE, WeaponInterface.SWORD) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(2876));
			target.performGraphic(new Graphic(1333, GraphicHeight.LOW));
	...
	ZAMORAK_GODSWORD(new int[] { 23630 }, 20, 1, 1, CombatType.MELEE, WeaponInterface.TWO_HANDED_SWORD) {
		@Override
		public CombatContainer container(Player player, Character target) {
	...
	SARADOMIN_GODSWORD(new int[] { 23629 }, 20, 1, 1, CombatType.MELEE, WeaponInterface.TWO_HANDED_SWORD) {
		@Override
		public CombatContainer container(Player player, Character target) {
...
/*
	...
	VESTAS_SPEAR(new int[] { 13905, 13907, 19818 }, 50, 1.26, 1, CombatType.MELEE, WeaponInterface.SPEAR) {
		@Override
		public CombatContainer container(Player player, Character target) {
			...
			return new CombatContainer(player, target, 1, CombatType.MELEE, true) {
				...
				public void onHit(int damage, boolean accurate) {
					if (target.isPlayer()) {
						...
						player.getPacketSender().sendMessage("You take recoil damage.");// temp messages? possible think
						// of better ones.
					}
				}
			};
			...
			TaskManager.submit(new Task(1, player, false) {
				...
				public void execute() {
					...
					new CombatHit(player.getCombatBuilder(),
							new CombatContainer(player, target, CombatType.RANGED, true)).handleAttack();
			...
			return new CombatContainer(player, target, 1, CombatType.MELEE, false) {
				...
				public void onHit(int damage, boolean accurate) {
					*//*
					 ...
					 *
					 * if (target.isPlayer()) { localEntities =
					 * Optional.of(player.getLocalPlayers()); } else if (target.isNpc()) {
					 * localEntities = Optional.of(player.getLocalNpcs()); }
					 *
					 * for (GameCharacter e : localEntities.get()) { if (e == null) { continue; }
					 *
					 * if (e.getPosition().isWithinDistance( target.getPosition(), 1) &&
					 * !e.equals(target) && !e.equals(player) && e.getConstitution() > 0 &&
					 * !e.isDead()) { Hit hit = CombatFactory.getHit(player, target,
					 * CombatType.MELEE); e.dealDamage(hit); e.getCombatBuilder().addDamage(player,
					 * hit.getDamage()); } } }
					 *//*
				}
			};
	...
	DINHS_BULWARK(new int[] { 11798 }, 100, 2, 1.8, CombatType.MELEE, WeaponInterface.UNARMED) {
		@Override
		public CombatContainer container(Player player, Character target) {
	...
	TSUK_SPEAR(new int[] { 18790 }, 25, 1, 1, CombatType.MELEE, WeaponInterface.SPEAR) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(1064));
			player.performGraphic(new Graphic(2599));
	...
	ZILYANA_SWORD(new int[] { 18792 }, 100, 1.35, 1.2, CombatType.MELEE, WeaponInterface.TWO_HANDED_SWORD) {
		@Override
		public CombatContainer container(Player player, Character target) {
	...
	BRUTAL_WHIP(new int[] { 4178, 20534, 20535, 22077,16835 }, 50, 1, 1, CombatType.MELEE, WeaponInterface.WHIP) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performGraphic(new Graphic(1928, GraphicHeight.HIGH));
			player.performAnimation(new Animation(2876));
			target.performGraphic(new Graphic(2108, GraphicHeight.HIGH));
			if (target.isPlayer()) {
				Player p = (Player) target;
	...
	NUETRON_DAGGER(new int[] { 15877 }, 25, 1.16, 1.20, CombatType.MELEE, WeaponInterface.DAGGER) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(1062));
			player.performGraphic(new Graphic(252, GraphicHeight.HIGH));
	...
	DEMON_MAUL(new int[] { 22078, 734, 22079, 22080 }, 50, 1.21, 1.30, CombatType.MELEE, WeaponInterface.WARHAMMER) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(10499));
	...
	KREE_CROSSBOW(new int[] { 18799 }, 33, 1.01, 2.01, CombatType.RANGED, WeaponInterface.ARMADYLXBOW) { // arma spec
		@Override
		public CombatContainer container(Player player, Character target) {
			...
			TaskManager.submit(new Task(1, player, false) {
				...
				public void execute() {
					...
					new Projectile(player, target, 1494, 44, 3, 0, 0, 0).sendProjectile();
					this.stop();
				}
			});
	...
	MINIGUN(new int[] { 19135, 19136, 19137, 20173, 671, 20553, 19843,18636,8088,8411,5012,5010,8001, 5011,22113,23474, 23568, 23222, 21606,8001,22089,9929,14056,15785,8253, 22083 }, 33, 1.01, 2.01, CombatType.RANGED,
	WeaponInterface.BSOAT) { // arma spec
		@Override
		public CombatContainer container(Player player, Character target) {

			player.performAnimation(new Animation(9308));

			TaskManager.submit(new Task(1, player, false) {
				@Override
				public void execute() {

					new Projectile(player, target, 999, 44, 3, 20, 20, 20).sendProjectile();
					this.stop();
				}
			});
	...
	KREE_GODSWORD(new int[] { 18835 }, 33, 1.43, 1.63, CombatType.MELEE, WeaponInterface.TWO_HANDED_SWORD) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(11989));
			player.performGraphic(new Graphic(2113));
			target.performGraphic(new Graphic(2113));
			return new CombatContainer(player, target, 3, CombatType.MELEE, true);
		}
	},

	STARLIGHT(new int[] { 22075 }, 50, 1.43, 1.63, CombatType.MELEE, WeaponInterface.LONGSWORD) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(6147));
			player.performGraphic(new Graphic(1052));
			target.performGraphic(new Graphic(1052));
			return new CombatContainer(player, target, 2, CombatType.MELEE, true);
		}
	},
	FLASHSPEC(new int[] { 18350,16045, 18365, 16043, 18356,19331, 18252, 18254, 20549 }, 50, 1.43, 1.63, CombatType.MELEE,
	WeaponInterface.LONGSWORD) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(4603));
			player.performGraphic(new Graphic(1928));
			target.performGraphic(new Graphic(1928));
			return new CombatContainer(player, target, 3, CombatType.MELEE, true);
		}
	},
	STUT_GODSWORD(new int[] { 18789 }, 33, 1.25, 1.4, CombatType.MELEE, WeaponInterface.TWO_HANDED_SWORD) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(7070));
			player.performGraphic(new Graphic(1662));
			target.performGraphic(new Graphic(1662));
	...
	ZILYANA_GODSWORD(new int[] { 18796 }, 33, 1.25, 1.5, CombatType.MELEE, WeaponInterface.TWO_HANDED_SWORD) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(10499));
			player.performGraphic(new Graphic(1521));
	...
	GENERAL_GODSWORD(new int[] { 18798 }, 33, 1.25, 1.4, CombatType.MELEE, WeaponInterface.TWO_HANDED_SWORD) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(11991));
			player.performGraphic(new Graphic(2232));
	...
	ARMADYL_CROSSBOW(new int[] { 22034, 18799, 18358 }, 40, 1.01, 2.01, CombatType.RANGED,
	WeaponInterface.ARMADYLXBOW) { // arma spec
		@Override
		public CombatContainer container(Player player, Character target) {
	...
	MORRIGANS_JAVELIN(new int[] { 13879, 13956 }, 50, 1.40, 1.30, CombatType.RANGED, WeaponInterface.JAVELIN) {
		@Override
		public CombatContainer container(Player player, Character target) {
	...
	ABYSSAL_WHIP(new int[] { 4151, 21371, 15441, 15442, 15443, 15444, 22008, 18686 }, 50, 1, 1, CombatType.MELEE,
	...
	/**
	 * Fired when the argued {@link Player} is about to attack the argued target.
	 * 
	...
	public static void drain(Player player, int amount) {
		...
		AchievementsOLD.finishAchievement(player, AchievementDataOLD.PERFORM_A_SPECIAL_ATTACK);
	}
