		...
		if (sucess) {
			if (imp.isRegistered()) {
				...
				if (implingData == ImpData.YOUNG)
					AchievementsOLD.finishAchievement(player, AchievementDataOLD.CATCH_A_YOUNG_IMPLING);
				else if (implingData == ImpData.KINGLY) {
					AchievementsOLD.doProgress(player, AchievementDataOLD.CATCH_5_KINGLY_IMPLINGS);
					AchievementsOLD.doProgress(player, AchievementDataOLD.CATCH_100_KINGLY_IMPLINGS);
				}
			}
		} else
