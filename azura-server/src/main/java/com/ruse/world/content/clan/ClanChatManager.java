...
public class ClanChatManager {
    ...
    public static void writeFile(ClanChat clan) {
        try {
            if (clan.getName().equalsIgnoreCase("Support")) {
                // System.out.println("Skipped writing Support.json file because it's read only.");
                return;
            }
            ...
            try (FileWriter writer = new FileWriter(file)) {
                ...
                object.addProperty("rank-required-to-enter",
                        (clan.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_ENTER] != null
                ...
                object.addProperty("rank-required-to-talk",
                        (clan.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_TALK] != null
                ...
                object.addProperty("rank-required-to-kick",
                ...
                object.addProperty("rank-required-to-visit-guild",
                        (clan.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_TALK] != null
                                ? clan.getRankRequirement()[ClanChat.RANK_REQUIRED_TO_TALK].ordinal()
                                : -1));
    ...
    public static void sendMessage(Player player, String message) {
        ...
        long playTime = player.getTotalPlayTime() + player.getRecordedLogin().elapsed();
        if (playTime < 1_800_000) {
            player.setCanChat(false);
            TaskManager.submit(new Task(9, false) {
                @Override
                public void execute() {
        ...
        for (Player memberPlayer : clan.getMembers()) {
            if (memberPlayer != null) {
                if (memberPlayer.getRelations().getIgnoreList().contains(player.getLongUsername()))
                    continue;
                int img = player.getRights() == PlayerRights.MANAGER ? 1541 : player.getRights() == PlayerRights.ZENYTE_DONATOR ? 1508
                ...
                if (player.getGameMode() == GameMode.IRONMAN) {
                    ...
                    ironimg = 839;
                } else if (player.getGameMode() == GameMode.EXTREME_MODE) {
                    ironimg = 838;
                ...
                memberPlayer.getPacketSender().sendMessage(":clan:" + formatted + "["
                        + clan.getName() + "]" + rankImg + "<img="
                        + ironimg + ">" + " " + NameUtils.capitalizeWords(player.getUsername()) + ": "
                        + /*
                 * chatColor +
        ...
        PlayerLogs.log(player.getUsername(),
                "(CC) " + (player.getCurrentClanChat() != null && player.getClanChatName() != null
                        ? player.getClanChatName()
                        : "NULL") + ". Said: " + StringUtils.capitalize(message.toLowerCase()));

	/*	DiscordMessager.sendClanMessage("(CC) "
				+ (player.getCurrentClanChat() != null && player.getClanChatName() != null ? player.getClanChatName()
				: "NULL")
				+ ". **" + player.getUsername() + "** said: " + StringUtils.capitalize(message.toLowerCase()));*/
    ...
    public static void logout(Player player) {
        ...
        for (int i = 0; i < 3; i++) {
            if (clan.getName().equalsIgnoreCase(clanGroups[i])) {
                clanPlayerCount[i] -= 1;
            }
        }
    }

        public static void leave(Player player, boolean kicked, boolean incrLeft) {
        final ClanChat clan = player.getCurrentClanChat();
        if (clan == null) {
    ...
    private static void join(Player player, ClanChat clan) {
        ...
        player.getPacketSender().sendMessage(" You're now talking in @red@" + clan.getOwnerName() + "</col> clan chat.");
    ...
    public static void delete(Player player) {
        try {
            ...
            for (Player member : clan.getMembers()) {
                if (member != null) {
                    leave(member, true, true);
                    member.setClanChatName("");
                }
            }
            if (player.getClanChatName() != null && clan.getName() != null
                    && player.getClanChatName().equalsIgnoreCase(clan.getName())) {
    ...
    public static void buttonClick(Player player, int id) { // check
        switch (id) {
       /*     case 29850:
            case 29852:
            case 29854:
                leave(player, false, false);
                ClanChatManager.join(player, clanGroups[(id - 29850) / 2]);
                updateClans(player);
                break;*/
    ...
    public static boolean inMainClans(String clan) { // check
           if (clan != null) {
               for (int i = 0; i < 3; i++) {
                   if (clan.equalsIgnoreCase(clanGroups[i])) {
                       return true;
                   }
               }
           }
    ...
    public static String[] clanGroups = new String[]{"Help", "Market", "Discussion", "No CC/Other"};
    public static int[] clanPlayerCount = new int[4];

    public static void updateClans(Player player) {
        if (player != null) {
            boolean listed = false;
            for (int i = 0; i < 3; i++) {
                if (player.getClanChatName() != null && player.getClanChatName().equalsIgnoreCase(clanGroups[i])) {
                    listed = true;
                    player.getPacketSender().sendString(29850 + (i * 2), "@whi@" + clanGroups[i]);
                } else {
                    player.getPacketSender().sendString(29850 + (i * 2), clanGroups[i]);
                }
                player.getPacketSender().sendString(29851 + (i * 2), clanPlayerCount[i] + " ppl");
            }
            player.getPacketSender().sendString(29856, (!listed ? "@whi@" : "") + clanGroups[3]);
            player.getPacketSender().sendString(29857, "---");

            if (player.getCurrentClanChat() == null)
                player.getPacketSender().sendString(29849, "Clan Size: @whi@N/A");
            else
                player.getPacketSender().sendString(29849, "Clan Size: @whi@(" + player.getCurrentClanChat().getMembers().size() + "/700)");
        }
    }
    ...
    public static boolean handleClanChatSetupButton(Player player, int id) { // check
        if (player.getInterfaceId() == 40172) {
            ...
            switch (id) {
                 ...
                 * +" just clicked the button to set joining to "+ClanChatRank.forId(l));
                 * clan.setRankRequirements(ClanChat.RANK_REQUIRED_TO_ENTER,
                 * ClanChatRank.forId(l)); clanChatSetupInterface(player, false);
                 * writeFile(clan); return true;
                 */
                case -18278:
                case -17519:
                case -17520:
                ...
                case -17526:
                    ...
                    // System.out.println(
                    //	player.getUsername() + " just clicked the button to set rank 2 talk" + ClanChatRank.forId(l));
                    player.getPacketSender().sendMessage("You have changed your clanchat channel's settings.");
                    clanChatSetupInterface(player, false);
                    writeFile(clan);
                    return true;
                ...
                case -17515:
                    ...
                    // System.out.println(
                    //player.getUsername() + " just clicked the button to set rank to kick " + ClanChatRank.forId(l));
                    player.getPacketSender().sendMessage("You have changed your clanchat channel's settings.");
                    clanChatSetupInterface(player, false);
                    updateList(clan);
                    writeFile(clan);
