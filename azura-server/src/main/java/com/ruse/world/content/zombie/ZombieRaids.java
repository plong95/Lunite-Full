...
public class ZombieRaids {
    ...
    public static void start(ZombieParty party) {
        ...
        party.setKills(0);
        party.sendMessage("@red@Welcome to Island Raids");
                ...
                TaskManager.submit(new Task(1, false) {
                    ...
                    protected void execute() {
                        if ((party.getOwner().getLocation() != Locations.Location.ZOMBIE)
                            ...
                            party.sendMessage("@red@Your party has failed to defeat the Island Raids");
                        ...
                        if (tick == 4) {
                            if (wave == 1)
                                party.sendMessage("@red@The Island Raids has started!");
                            else if (wave == 2)
    ...
    public static void finishRaid(ZombieParty party) {
        ...
        TaskManager.submit(new Task(3, false) {
            ...
            @Override
            public void execute() {
                party.sendMessage("@red@Your party has defeated the Island Raids");
                ...
                for (Player player : party.getPlayers()) {
                    ...
                    if (drop.isAnnounce()) {
                        String message = "@blu@News: @red@" + player.getUsername() + " @blu@has just received @red@"
                                + ItemDefinition.forId(drop.getId()).getName() + "@blu@ from @red@Island Raids";
                    ...
                    if (player.getInventory().getFreeSlots() == 0 && !player.getInventory().contains(18404)){
                        player.depositItemBank(new Item(drop.getId(), (int) amt));
                        player.sendMessage("Your reward has been sent to your bank");
                    }else{
                        player.getInventory().add(new Item(drop.getId(), (int) amt));
                    }
                    party.moveTo(ZombieRaidData.lobbyPosition);
                    player.sendMessage("<shad=1>@yel@You have received X" + (int) amt + " "+ ItemDefinition.forId(drop.getId()).getName() + " from this raid!" );
