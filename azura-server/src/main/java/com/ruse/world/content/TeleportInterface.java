...
public class TeleportInterface {
    ...
    public static void handleDungeonsTeleport(Player player, Dungeons wildyData) {
        ...
        if (wildyData == Dungeons.HOV) {
            player.hov.initArea();
            player.hov.start();
            player.getPacketSender().sendInterfaceRemoval();
            return;
        }
        if (wildyData == Dungeons.VOD) {
            player.vod.initArea();
            player.vod.start();
            player.getPacketSender().sendInterfaceRemoval();
            return;
        }
        TeleportHandler.teleportPlayer(player, new Position(wildyData.teleportCords[0],
                wildyData.teleportCords[1], wildyData.teleportCords[2]), player.getSpellbook().getTeleportType());
    }
    ...
    public static void handleMinigameTeleport(Player player, Minigames minigameData) {
        ...
        TeleportHandler.teleportPlayer(player,
                new Position(minigameData.teleportCords[0], minigameData.teleportCords[1], minigameData.teleportCords[2]),
                player.getSpellbook().getTeleportType());
    }
    ...
    public static void sendBossData(Player player, Bosses data) {
        ...
        player.getPacketSender().sendString(122016, "Max Hit: " + NpcDefinition.forId(data.npcId).getMaxHit() + "");
        player.getPacketSender().sendString(122017, "Hitpoints: " + NpcDefinition.forId(data.npcId).getHitpoints() + "");
    ...
    public static void showList(Player player, Teleport[] list) {
        ...
        player.getPacketSender().sendInterface(122000);
        player.getPacketSender().sendString(122019, "TELEPORT");
    }
    ...
    public static void sendTitles(Player player) {
        String[] categories = new String[]{"Monsters", "Bosses", "Raids", "Dungeons", "Skilling"};
        for (int i = 0; i < 5; i++) {
    ...
    public static void sendDrops(Player player, Box[] drops) {
        try {
            if (drops != null) {
                ...
                for (int i = 0; i < length + 5; i++) {
                    if (drops.length > i)
                        player.getPacketSender().sendItemOnInterface1(51251, drops[i].getId(), i,
                                drops[i].getAmount());
                    else
                        player.getPacketSender().sendItemOnInterface1(51251, -1, i,
                                0);
                }
                int scroll = 7 + (length / 5) * 35;
                if (scroll <= 43)
                    scroll = 43;
                player.getPacketSender().setScrollBar(51250, scroll);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public enum Monsters implements Teleport {
        ...
        VAMP("Creepy Crawlers", 1703, new int[]{1815, 4909, 0}, 700),
        JELLY("Lil Stinger", 1721, new int[]{2889, 4015, 0}, 800),
        //JELLO("Cuber", 1729, new int[]{1878, 4908, 0}, 800),
        //BUNYIP("Bunyip", 1705, new int[]{1903, 4879, 0}, 800),
        GARG("Bronze Demon", 1712, new int[]{1904, 4850, 0}, 1500),
        BUTTERFLY("Fly on Fire", 1711, new int[]{1880, 4822, 0}, 800),
        CLOUD("Menace", 1739, new int[]{1846, 4819, 0}, 1200),
        BLOODVELD("Soul Sucker", 1710, new int[]{1809, 4834, 0}, 800),
        LAVANNOTH("Assanoth", 1702, new int[]{1829, 4863, 0}, 1000),
        CRAB("Rune Crab", 1700, new int[]{2925, 4016, 0}, 800),
        ANT("Buggy", 1724, new int[]{2928, 3982, 0}, 700),
        //MOSQUITO("Mosquito", 1713, new int[]{2025, 4772, 0}, 800),
        //PLANT("Murder Plant", 1737, new int[]{2026, 4759, 0}, 800),

        UNICORN("Zombiecorn", 1742, new int[]{2212, 4941, 0}, 1200),
        DRAGON("Baby Draco", 1706, new int[]{2187, 4943, 0}, 800),
        Z_BIRD("Ugly Bird", 1725, new int[]{2214, 4960, 0}, 800),
        GROOTER("Groot", 1744, new int[]{2166, 4946, 0}, 1200),
    ...
    public enum Bosses implements Teleport {
        ELITE_DRAGON("Spartan dragon", 8015, new int[]{2895, 3982, 0}, 1500),
        ETERNAL_DRAGON("Sneaky dragon", 4972, new int[]{2975, 4000, 0}, 2500),
        SCARLET_FALCON("Malevolent Bird", 2949, new int[]{3036, 4003, 0}, 2500),

        CRYSTAL_QUEEN("Crysty", 6430, new int[]{1887, 5468, 0}, 2500),
        LUCIFER("Molten Death", 9012, new int[]{2335, 3998, 0}, 2500),
        MEGA_AVATAR("Ninja Avatar", 4540, new int[]{1884, 5334, 0}, 2500),

        LIGHT_SUPREME("Bringer of Light", 440, new int[]{2761, 4575, 0}, 2000),
        DARK_SUPREME("Taker of Darkness", 438, new int[]{2721, 4450, 0}, 2500),
        FRACTITE_DEMON("Ice demon", 12843, new int[]{2785, 4525, 0}, 2000),
        INFERNAL_DEMON("Death demon", 12810, new int[]{2712, 4508, 0}, 2500),
        CRAZY_WITCH("Slither", 1234, new int[]{2784, 4445, 0}, 1000),

        GLOBAL_BOSSES("Global boss", 9014, new int[]{2139, 5019, 0}, 1000),
    ...
    public enum Minigames implements Teleport {
        CRYSTAL_RAID("Crystal Raids", 51, new int[]{2723, 2737, 0}, 1200),
        //TH("Cursed Hunt", 9816, new int[]{2015, 5022, 0}, 3000),
        //KOL("Keepers of Light", 9835, new int[]{2322, 5028, 0}, 1200),
        //VOW("Vault of War", 9839, new int[]{1776, 5335, 0}, 1200),
        ;
    ...
    public enum Dungeons implements Teleport {
        DUNG("Dungeoneering", 11226, new int[]{2251, 5040, 0}, 700),
        HOV("Forsaken Dungeon", 9024, new int[]{2195, 5037, 0}, 1400),
        VOD("Death of Trinity", 9028, new int[]{1954, 5010, 0}, 600),
    ...
    public enum Misc implements Teleport {
        ...
        STARTER_ZONE("Starter Zone", 9001, new int[]{1, 1, 0}, 600),
        SKILL_ISLAND("Skilling Zone", 805, new int[]{2817, 2600, 0}, 700),
