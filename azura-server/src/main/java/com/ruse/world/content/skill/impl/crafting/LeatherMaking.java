		...
		for (final leatherData l : leatherData.values()) {
			...
			if (leather == l.getLeather()) {
				if (l.getLeather() == 1741) {
					...
					player.getPacketSender().sendString(2799, ItemDefinition.forId(1131).getName())
	...
	public static void craftLeather(final Player player, final leatherData l, final int amount) {
		...
		if (l.getLeather() == player.getSelectedSkillingItem()) {
			...
			if (player.getInventory().getAmount(l.getLeather()) < l.getHideAmount()) {
				player.getPacketSender().sendMessage("You need some "
						+ ItemDefinition.forId(l.getLeather()).getName().toLowerCase() + " to make this item.");
				player.getPacketSender().sendInterfaceRemoval();
				return;
			}
			player.setCurrentTask(new Task(2, player, true) {
				...
				@Override
				public void execute() {
					if (!player.getInventory().contains(1734) || !player.getInventory().contains(1733)
							|| player.getInventory().getAmount(l.getLeather()) < l.getHideAmount()) {
					...
					if (l == leatherData.LEATHER_BOOTS) {
						AchievementsOLD.finishAchievement(player, AchievementDataOLD.CRAFT_A_PAIR_OF_LEATHER_BOOTS);
					} else if (l == leatherData.BLACK_DHIDE_BODY) {
						AchievementsOLD.doProgress(player, AchievementDataOLD.CRAFT_20_BLACK_DHIDE_BODIES);
					}
