...
public class DwarfMultiCannon {
	...
	CANNON_BASE = 7, CANNON_STAND = 8, CANNON_BARRELS = 9, CANNON = 6,
			/** Start of cannon item ids **/
			CANNONBALL = 2, CANNON_BASE_ID = 6, CANNON_STAND_ID = 8, CANNON_BARRELS_ID = 10, CANNON_FURNACE_ID = 12;
	...
	public static void setupCannon(final Player c) {
		...
		TaskManager.submit(new Task(2, c, true) {
			...
			public void execute() {
				...
				case 1:
					setupTicks = 5;
					AchievementsOLD.finishAchievement(c, AchievementDataOLD.SET_UP_A_CANNON);
	...
	public static void startFiringCannon(Player c, DwarfCannon cannon) {
		...
		if (cannon.getCannonballs() <= 15) {
			...
			if (playerCannonballs < 1) {
				c.getPacketSender()
						.sendMessage("You do not have any cannonballs in your inventory to fire the cannon with.");
				return;
			}
	...
	public static void attack(Player player) { // TODO: Bad hardcoding of setting damage, switch to combathittask
		...
		AchievementsOLD.doProgress(player, AchievementDataOLD.DEAL_EASY_DAMAGE_USING_RANGED, (int) dmg.getDamage());
		AchievementsOLD.doProgress(player, AchievementDataOLD.DEAL_MEDIUM_DAMAGE_USING_RANGED, (int) dmg.getDamage());
		AchievementsOLD.doProgress(player, AchievementDataOLD.DEAL_HARD_DAMAGE_USING_RANGED, (int) dmg.getDamage());
		...
		if (cannon.getCannonballs() == 15) {
			player.getPacketSender().sendMessage(MessageType.NPC_ALERT,
					"Your cannon has " + cannon.getCannonballs() + " Cannonballs left.");
		}
	...
	private static NPC getTarget(Player p, DwarfCannon cannon) {
		for (NPC n : p.getLocalNpcs()) {
			...
			if (n.getDefinition().isAttackable() && n.getConstitution() > 0
					&& Locations.goodDistance(cannon.getObject().getPosition(), n.getPosition(), 8)) {
				if (!Location.inMulti(n) && n.getCombatBuilder().isBeingAttacked()
						&& n.getCombatBuilder().getLastAttacker() != p) {
					continue;
				}
