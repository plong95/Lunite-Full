...
public class CurrencyPouch {
    ...
    private long bossSlayerTickets;
    private long instanceTokenUntradeable;
    private long celestialEnergy;
    private long highTierTickets;
    ...
    public static void checkBalance(Player player) {
        ...
        amount = player.getCurrencyPouch().bossSlayerTickets;
        currency = "Boss Slayer Tickets";
        player.sendMessage("Your pouch contains @red@" + Misc.formatNumber(amount) + " @bla@" + currency);

        amount = player.getCurrencyPouch().instanceTokenUntradeable;
        currency = "Instance Token (u)";
        player.sendMessage("Your pouch contains @red@" + Misc.formatNumber(amount) + " @bla@" + currency);
        ...
        amount = player.getCurrencyPouch().highTierTickets;
    ...
    public static void withdrawCurrency(Player player, int type, int amount) {
        ...
        switch (type) {
            ...
            case 11: //Boss Slayer Ticket
                currency = 621;
                inventoryAmount = player.getInventory().getAmount(currency);

                if (player.getCurrencyPouch().bossSlayerTickets < amount) {
                    insufficientFundsError = true;
                    break;
                }

                if (player.getCurrencyPouch().bossSlayerTickets <= 0) {
                    emptyFundsError = true;
                    break;
                }
                ...
                if (player.getCurrencyPouch().bossSlayerTickets >= amount) {
                    ...
                    player.getCurrencyPouch().bossSlayerTickets -= amount;
                    player.getInventory().add(new Item(currency, amount));
                }
                break;
            case 12: //Instance Token U
                currency = 23211;
                inventoryAmount = player.getInventory().getAmount(currency);

                if (player.getCurrencyPouch().instanceTokenUntradeable < amount) {
                    insufficientFundsError = true;
                    break;
                }

                if (player.getCurrencyPouch().instanceTokenUntradeable <= 0) {
                    emptyFundsError = true;
                    break;
                }
                ...
                if (player.getCurrencyPouch().instanceTokenUntradeable >= amount) {
                    ...
                    player.getCurrencyPouch().instanceTokenUntradeable -= amount;
                    player.getInventory().add(new Item(currency, amount));
                }
                break;
            case 13: //Celestial Energy
                ...
                break;
            case 14: //Celestial Energy
                currency = 23456;
                inventoryAmount = player.getInventory().getAmount(currency);

                if (player.getCurrencyPouch().highTierTickets < amount) {
                    insufficientFundsError = true;
                    break;
                }

                if (player.getCurrencyPouch().highTierTickets <= 0) {
                    emptyFundsError = true;
                    break;
                }
                ...
                if (player.getCurrencyPouch().highTierTickets >= amount) {
                    ...
                    player.getCurrencyPouch().highTierTickets -= amount;
                    player.getInventory().add(new Item(currency, amount));
                }
                break;
        }
    ...
    public static void depositCurrency(Player player, int type, int amount) {
        ...
        switch (type) {
            ...
            case 11: //Boss Slayer Ticket
                currency = 621;
                break;
            case 12: //Instance Tokens Untradeable
                currency = 23211;
                break;
            case 13: //Celestial Energy
                currency = 23736;
                break;
            case 14: //High-Tier Tickets
                currency = 23456;
                break;
            default:
    ...
    private static boolean attemptDeposit(Player player, int amount, boolean insufficientFundsError, int currency) {
        ...
        if (currency == 12855) {
            ...
            player.getCurrencyPouch().blueCharms += amount;
        } else if (currency == 621) {
            player.getCurrencyPouch().bossSlayerTickets += amount;
        } else if (currency == 23211) {
            player.getCurrencyPouch().instanceTokenUntradeable += amount;
        } else if (currency == 23736) {
            player.getCurrencyPouch().celestialEnergy += amount;
        } else if (currency == 23456) {
            player.getCurrencyPouch().highTierTickets += amount;
        }
