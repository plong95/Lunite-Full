			...
			public void execute() {
				...
				if (!CookingData.success(player, 3, fish.getLevelReq(), fish.getStopBurn())) {
					...
					if (fish == CookingData.SALMON) {
						AchievementsOLD.finishAchievement(player, AchievementDataOLD.COOK_A_SALMON);

					} else if (fish == CookingData.ROCKTAIL) {
						AchievementsOLD.doProgress(player, AchievementDataOLD.COOK_25_ROCKTAILS);
						AchievementsOLD.doProgress(player, AchievementDataOLD.COOK_1000_ROCKTAILS);
					} else if (fish == CookingData.SHARK) {
						Achievements.doProgress(player, Achievements.Achievement.DAILY_DINNER);
					}
					Achievements.doProgress(player, Achievements.Achievement.COOK_ALOT);
					Achievements.doProgress(player, Achievements.Achievement.COOKING);
