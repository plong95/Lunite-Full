...
public class MoneyPouch {
	...
	public static final int BUCKS_ID = 5021; // item id
	public static final long BUCKS_VALUE = 1000000; // 1b
	...
	public static boolean depositTickets(Player plr, long amount) {
		if (amount <= 0)
			return false;
		if (plr.getInterfaceId() > 0) {
			plr.getPacketSender().sendMessage("Please close the interface you have open before opening another one.");
		...
		if (validateTicketAmount(plr, amount)) {
			long addedMoney = (long) plr.getMoneyInPouch() + (long)( amount* 1000000);
			if (addedMoney > Long.MAX_VALUE) {
				long canStore = Long.MAX_VALUE - plr.getMoneyInPouch();
				plr.getInventory().delete(5021, (int) canStore);
				plr.setMoneyInPouch(plr.getMoneyInPouch() + (canStore * 1000000));
				plr.getPacketSender().sendString(8135, "" + plr.getMoneyInPouch());
				plr.getPacketSender().sendMessage("You've added " + canStore + " coins to your money pouch.");
				return true;
			} else {
				plr.getInventory().delete(5021, (int) amount);
				plr.setMoneyInPouch(plr.getMoneyInPouch() + (amount * 1000000));
				plr.getPacketSender().sendString(8135, "" + plr.getMoneyInPouch());
				plr.getPacketSender().sendMessage("You've added " + amount + " 1M tickets to your money pouch.");
				return true;
			}
		} else {
			plr.getPacketSender().sendMessage("You do not seem to have " + amount + " 1M tickets in your inventory.");
			return false;
		}
	}
	...
	private static boolean validateTicketAmount(Player plr, long amount) {
		return plr.getInventory().getAmount(5021) >= amount;
	}
}