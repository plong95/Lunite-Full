...
public class CurseHandler {
	...
	public static void activateCurse(Player player, CurseData curse) {
		...
		if (curse == CurseData.SOUL_SPLIT && player.getLocation() == Location.DEATH_SANCTUM && player.getPosition().getZ() != 0) {
			player.getPacketSender().sendMessage("You cannot use soulsplit here.");
			CurseHandler.deactivateAll(player);
			PrayerHandler.deactivateAll(player);
			return;
		}
		if (curse == CurseData.SOUL_SPLIT && player.getLocation() == Location.ANIMA_CHAMBERS && player.getPosition().getZ() != 0) {
			player.getPacketSender().sendMessage("You cannot use soulsplit here.");
			CurseHandler.deactivateAll(player);
			PrayerHandler.deactivateAll(player);
			return;
		...
		if (curse == CurseData.SOUL_SPLIT && player.getLocation() == Location.ISLE_GODS && player.getPosition().getZ() != 0) {
			player.getPacketSender().sendMessage("You cannot use soulsplit here.");
			CurseHandler.deactivateAll(player);
			PrayerHandler.deactivateAll(player);
			return;
		...
		TaskManager.submit(new Task(1, player, true) {
			...
			public void execute() {
				...
				if(player.getEquipment().get(Equipment.CAPE_SLOT).getId() == 1486 ||
						...
						|| player.getEquipment().get(Equipment.CAPE_SLOT).getId() ==  23717
						|| player.getEquipment().get(Equipment.CAPE_SLOT).getId() == 23689
						|| player.getEquipment().get(Equipment.CAPE_SLOT).getId() == 23620
						|| player.getEquipment().get(Equipment.CAPE_SLOT).getId() == 23595) {
					drain = 0;
				}
