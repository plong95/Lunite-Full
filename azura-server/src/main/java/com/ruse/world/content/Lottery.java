...
public class Lottery {
	...
	public static void enterLottery(Player p) {
		...
		if (p.getInventory().getAmount(995) < PRICE_TO_ENTER && !usePouch || p.getRights() == PlayerRights.DEVELOPER
				|| p.getRights() == PlayerRights.ONYX_DONATOR
				|| p.getRights() == PlayerRights.ZENYTE_DONATOR) {
	...
	/**
	 * Adds a user to the lottery by writing their username to the file aswell as
	 * adding them to the list of users who have entered already.
	 * 
	...
	public static void init() {
		try {
			Misc.createFilesIfNotExist(CONTESTERS_FILE_LOCATION.getPath(), false);
			...
			while (true) {
				...
				if (line.length() > 0) {
					if (!CONTESTERS.contains(line)) // user might have gotten on list twice somehow.. don't give them
													// extra chance of winning
						CONTESTERS.add(line);
				}
			}
			r.close();
			Misc.createFilesIfNotExist(LAST_WINNER_FILE_LOCATION.getPath(), false);
	...
	public static void restartLottery() {
		...
		try {
			...
			if (winner != null) {
				...
				World.sendMessage(
						"<col=D9D919><shad=0>The lottery needs some more contesters before a winner can be selected.");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	/**
	 * Handles the lottery for a player on login Checks if a user won the lottery
