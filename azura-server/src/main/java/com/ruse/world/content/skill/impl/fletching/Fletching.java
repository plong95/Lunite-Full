...
public class Fletching {
	 ...
	 * 
	 * @param player The Player clicking a button The button the player clicked
	 * @return
	 */
	...
	public static void tipBolt(final Player player, int tip) {
		...
		player.getPacketSender()
				.sendMessage("You begin fletching " + ItemDefinition.forId(bd.getOutcome()).getName() + ".");
		...
		if (arr != null) {
			if (player.getSkillManager().getCurrentLevel(Skill.FLETCHING) >= arr.getLevelReq()) {
				if (player.getInventory().getAmount(arr.getItem1()) >= 15
					...
					AchievementsOLD.finishAchievement(player, AchievementDataOLD.FLETCH_SOME_ARROWS);

					Achievements.doProgress(player, Achievements.Achievement.FLETCHING);
					
					if (arr == ArrowData.RUNE) {
						AchievementsOLD.doProgress(player, AchievementDataOLD.FLETCH_450_RUNE_ARROWS, 15);
						AchievementsOLD.doProgress(player, AchievementDataOLD.FLETCH_5000_RUNE_ARROWS, 15);
					}
				} else {
