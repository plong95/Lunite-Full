    ...
    public static class CombatHit {
        ...
        public void handleAttack() {
            ...
            if (container.getHits().length != 0 && container.getCombatType() != CombatType.MAGIC
                ...
                if (attacker.isPlayer()) {
                    ...
                    if (damage > 0) {
                        ...
                        if (p.getLocation() == Location.DEATH_SANCTUM) {
                            p.setRaidsDamage(p.getRaidsDamage() + damage);
                        }
                        if (p.getLocation() == Location.ANIMA_CHAMBERS) {
                            p.setRaidsDamage(p.getRaidsDamage() + damage);
                            if (p.getChambersOfAnimaParty() != null) {
                                if (damage > 0){
                                    p.getChambersOfAnimaParty().incrementDamage(damage);
                                }
                            }
                        }
                        if (damage >= 2500000 ){
                        ...
                        if (container.getCombatType() == CombatType.MELEE) {
                            Achievements.doProgress(p, Achievements.Achievement.DEAL_100K_MELEE_DMG, damage);
                            Achievements.doProgress(p, Achievements.Achievement.DEAL_10M_MELEE_DMG, damage);
                            Achievements.doProgress(p, Achievements.Achievement.DEAL_100M_MELEE_DMG, damage);
                            Achievements.doProgress(p, Achievements.Achievement.DEAL_1B_MELEE_DMG, damage);

                            AchievementsOLD.doProgress(p, AchievementDataOLD.DEAL_EASY_DAMAGE_USING_MELEE, damage);
                            AchievementsOLD.doProgress(p, AchievementDataOLD.DEAL_MEDIUM_DAMAGE_USING_MELEE, damage);
                            AchievementsOLD.doProgress(p, AchievementDataOLD.DEAL_HARD_DAMAGE_USING_MELEE, damage);
                        } else if (container.getCombatType() == CombatType.RANGED) {
                            Achievements.doProgress(p, Achievements.Achievement.DEAL_100K_RANGE_DMG, damage);
                            Achievements.doProgress(p, Achievements.Achievement.DEAL_10M_RANGED_DMG, damage);
                            Achievements.doProgress(p, Achievements.Achievement.DEAL_100M_RANGED_DMG, damage);
                            Achievements.doProgress(p, Achievements.Achievement.DEAL_1B_RANGED_DMG, damage);

                            AchievementsOLD.doProgress(p, AchievementDataOLD.DEAL_EASY_DAMAGE_USING_RANGED, damage);
                            AchievementsOLD.doProgress(p, AchievementDataOLD.DEAL_MEDIUM_DAMAGE_USING_RANGED, damage);
                            AchievementsOLD.doProgress(p, AchievementDataOLD.DEAL_HARD_DAMAGE_USING_RANGED, damage);
                        } else if (container.getCombatType() == CombatType.MAGIC) {
                            Achievements.doProgress(p, Achievements.Achievement.DEAL_100K_MAGIC_DMG, damage);
                            Achievements.doProgress(p, Achievements.Achievement.DEAL_10M_MAGE_DMG, damage);
                            Achievements.doProgress(p, Achievements.Achievement.DEAL_100M_MAGE_DMG, damage);
                            Achievements.doProgress(p, Achievements.Achievement.DEAL_1B_MAGE_DMG, damage);

                            AchievementsOLD.doProgress(p, AchievementDataOLD.DEAL_EASY_DAMAGE_USING_MAGIC, damage);
                            AchievementsOLD.doProgress(p, AchievementDataOLD.DEAL_MEDIUM_DAMAGE_USING_MAGIC, damage);
                            AchievementsOLD.doProgress(p, AchievementDataOLD.DEAL_HARD_DAMAGE_USING_MAGIC, damage);
                        }
                        ...
                        if (victim.isPlayer()) {
                            AchievementsOLD.finishAchievement(p, AchievementDataOLD.FIGHT_ANOTHER_PLAYER);
                        }
                    }
                } else {
            ...
            if (!container.isAccurate()) {
                ...
                if (attacker.isPlayer()) {
                    if (SpecialAmmo.isAmmo((Player) attacker, SpecialAmmo.HEALING)) {
                        int heal = (int) (damage * SpecialAmmo.getAmmo((Player) attacker).getBoost());
                        if (heal <= 5)
                            heal = 5;
                        if (heal >= 60)
                            heal = 60;
                    ...
                    if (((Player) attacker).getEquipment().contains(23705)
                    || ((Player) attacker).getEquipment().contains(23742)) {
                        int heal = (int) (damage * 0.003);
                        if (heal <= 5)
                            heal = 5;
                        if (heal >= 60)
                            heal = 60;
        ...
        public void retaliate() {
            if (victim.isPlayer()) {
                ...
                NPC npc = (NPC) victim;
                if (npc.getId() != 10000 && npc.getId() != 10001) {
