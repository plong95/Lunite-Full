...
public class Runecrafting {
	...
	public static void craftRunes(final Player player, RuneData rune) {
		...
		if (rune == RuneData.BLOOD_RUNE) {
			AchievementsOLD.doProgress(player, AchievementDataOLD.RUNECRAFT_500_BLOOD_RUNES, amountMade);
			AchievementsOLD.doProgress(player, AchievementDataOLD.RUNECRAFT_8000_BLOOD_RUNES, amountMade);
		}
		Achievements.doProgress(player, Achievements.Achievement.RUNECRAFTING);

		player.performGraphic(new Graphic(129));
		player.getSkillManager().addExperience(Skill.RUNECRAFTING, 30); 
		player.getSkillManager().addExperience(Skill.RUNECRAFTING, rune.getXP());
		...
		if (player.getSkillManager().skillCape(Skill.RUNECRAFTING) && saved > 0) {
			player.getPacketSender().sendMessage(
					"Your cape has recycled " + saved + " essence into " + amountToMake * saved + " runes.");
			saved = 0;
		}
		AchievementsOLD.finishAchievement(player, AchievementDataOLD.RUNECRAFT_SOME_RUNES);
		player.getClickDelay().reset();
	}

	public static void handleTalisman(Player player, int ID) {
		TalismanData talisman = TalismanData.forId(ID);
		if (talisman == null)
			return;
