...
public class Barrows {
	...
	public static boolean handleObject(final Player player, GameObject object) {
		switch (object.getId()) {
		case 6771:
			searchCoffin(player, object.getId(), 4, 2026,
					object.getPosition() != null ? new Position(3557, 9715, player.getPosition().getZ())
							: new Position(3552, 9693));
			return true;
		case 6823:
			searchCoffin(player, object.getId(), 0, 2030,
					object.getPosition() != null ? new Position(3575, 9704, player.getPosition().getZ())
							: new Position(3552, 9693));
			return true;
		case 6821:
			searchCoffin(player, object.getId(), 5, 2025,
					object.getPosition() != null ? new Position(3557, 9699, player.getPosition().getZ())
							: new Position(3552, 9693));
			return true;
		case 6772:
			searchCoffin(player, object.getId(), 1, 2029,
					object.getPosition() != null ? new Position(3571, 9684, player.getPosition().getZ())
							: new Position(3552, 9693));
			return true;
		case 6822:
			searchCoffin(player, object.getId(), 2, 2028,
					object.getPosition() != null ? new Position(3549, 9681, player.getPosition().getZ())
							: new Position(3552, 9693));
			return true;
		case 6773:
			searchCoffin(player, object.getId(), 3, 2027,
					object.getPosition() != null ? new Position(3537, 9703, player.getPosition().getZ())
							: new Position(3552, 9693));
			return true;
		case 6745:
		...
		case 10284:
			...
			if (player.getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[player
				...
				handleObject(player, new GameObject(COFFIN_AND_BROTHERS[player.getMinigameAttributes()
				...
				player.getPacketSender().sendMessage("<img=5><shad=0> @lre@You've looted a total of "
						+ player.getPointsHandler().getBarrowsChests() + " Barrows chests!");
				if (player.getPointsHandler().getBarrowsPoints() == 10) {
					player.getPacketSender()
							.sendMessage("<img=5> @red@You can now purchase the \"Looter\" loyalty title.");
				}
				if (player.getPointsHandler().getBarrowsChests() < 10) {
					player.getPacketSender()
							.sendMessage("<img=5><shad=0>Only " + (10 - player.getPointsHandler().getBarrowsChests())
									+ " more for the @lre@\"@red@Looter@lre@\" title.");
				}
	 ...
	 * @param obj                                    The object (coffin) being
	 *                                               searched
	 * @param coffinId                               The coffin's array index
	 * @param npcId                                  The NPC Id of the NPC to spawn
	 *                                               after searching
	...
	public static void updateInterface(Player player) {
		...
		player.getPacketSender().sendString(37208,
				"Killcount: " + player.getMinigameAttributes().getBarrowsMinigameAttributes().getKillcount());
	}
