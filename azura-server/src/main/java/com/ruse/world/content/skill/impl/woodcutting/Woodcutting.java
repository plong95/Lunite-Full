...
import com.ruse.world.content.WaveMinigame;
			...
			if (player.getSkillManager().getCurrentLevel(Skill.WOODCUTTING) >= h.getRequiredLevel()) {
				...
				if (t != null) {
					...
					if (player.getSkillManager().getCurrentLevel(
						...
						if (h == Hatchet.CRYSTAL && delay >= 1)
							delay -= 1;

						int finalDelay = delay;
						player.setCurrentTask(new Task(1, player, false) {
							int cycle = 0, reqCycle = finalDelay >= 2 ? finalDelay : Misc.getRandom(1) + 1;
							...
							public void execute() {
								...
								if (cycle != reqCycle) {
									...
									if (player.getLocation() == Locations.Location.WAVE_MINIGAME){
										if (player.getWaveMinigame().getTask() == WaveMinigame.Tasks.CHOP_WOOD){
											...
											if (Misc.getRandom(10) <= 6) {
												...
												if (fmLog != null) { // if their their logdata is not null...
													...
													if (fmLog == logData.OAK) {
														AchievementsOLD.finishAchievement(player,
																AchievementDataOLD.BURN_AN_OAK_LOG);
													} else if (fmLog == logData.MAGIC) {

														AchievementsOLD.doProgress(player,
																AchievementDataOLD.BURN_100_MAGIC_LOGS);
														AchievementsOLD.doProgress(player,
																AchievementDataOLD.BURN_2500_MAGIC_LOGS);
													}
									...
									if (t != null && t == Trees.OAK) {
										...
										Achievements.doProgress(player, Achievements.Achievement.DAILY_CHOPPING);
									}
									Achievements.doProgress(player, Achievements.Achievement.CHOP_ALOT);
									Achievements.doProgress(player, Achievements.Achievement.WOODCUTTING);
