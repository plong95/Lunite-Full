...
public class CombatBuilder {
	...
	public void attack(Character target) {
		...
		if (target.equals(victim)) {
			...
			int realDistance = strategy.getCombatType() == CombatType.MAGIC ? strategy.attackDistance(character)+1 : strategy.attackDistance(character);
			if (!character.getPosition().equals(victim.getPosition()) && character.getPosition()
					.isWithinDistance(victim.getPosition(), realDistance)) {
		...
		if (combatSession != null) {
			...
			if (character.isPlayer()) {
				Player player = (Player) character;
				if (AutoCastSpell.getAutoCastSpell(player) != null || player.getCastSpell() == null || attackTimer < 1) {
					cooldown = 0;
				}
			}
			return;
	...
	/**
	 * Resets this combat builder by discarding various values associated with the
	 * combat process.
	 */
	 ...
	 * 
	 * @param clearMap <code>true</code> if the map should be discarded once the
	...
	public List<DamageDealer> getTopKillers(NPC npc, int numKillers) {
		...
		for (int i = 0; i < numKillers; i++) {
			...
			// // System.out.println(damageDealer.getPlayer().getUsername() + " is killer " +
	...
	public static class CombatDamageCache {
		 ...
		 */
		public CombatDamageCache(long damage) {
			this.damage = damage;
		 ...
		 */
		public void incrementDamage(long damage) {
			this.damage += damage;
