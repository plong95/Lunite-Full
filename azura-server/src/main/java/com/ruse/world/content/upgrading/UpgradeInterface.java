...
public class UpgradeInterface {
    ...
    public boolean handleButton(int buttonId) {
        ...
        switch (buttonId) {
            ...
            case 121021:
                ...
                return true;
            case 121005:
                openInterface(Upgradeables.UpgradeType.TIER1);
                return true;
            case 121006:
                openInterface(Upgradeables.UpgradeType.TIER2);
                return true;
            case 121007:
                openInterface(Upgradeables.UpgradeType.TIER3);
                return true;
            case 121008:
                openInterface(Upgradeables.UpgradeType.TIER4);
                return true;
        }
                ...
                if (getRestrictions(val, all)) {
                    ...
                    if (all) {
                        ...
                        for (int i = 0; i < amount; i++) {
                            if (player.getInventory().contains(ItemDefinition.SCRAP_ID,
                                    val.getCost())) {
                                if (noted) {
                                ...
                                player.getInventory().delete(ItemDefinition.SCRAP_ID,
                                        val.getCost());
                        ...
                        player.getInventory().delete(ItemDefinition.SCRAP_ID, val.getCost());
    ...
    public double getBoost(double chance){
        ...
        if(player.getAmountDonated() >= Donation.VISIONARY_MEMBER || player.getRights().equals(PlayerRights.YOUTUBER)) {
            percentBoost += 15;
        } else if(player.getAmountDonated() >= Donation.DIAMOND_MEMBER) {
            percentBoost += 10;
        } else if(player.getAmountDonated() >= Donation.PLATINUM_MEMBER) {
            percentBoost += 7;
        } else if(player.getAmountDonated() >= Donation.GOLD_MEMBER) {
            percentBoost += 5;
        }
    ...
    private boolean getRestrictions(Upgradeables data, boolean all) {
        ...
        if (noted && player.getInventory().contains(data.getRequired().getId() + 1)) {
            ...
            if (!player.getInventory().contains(data.getRequired().getId() + 1, amount) ||
                    !player.getInventory().contains(ItemDefinition.SCRAP_ID, amount *
                            data.getCost())) {
            ...
            if (!player.getInventory().contains(data.getRequired().getId()) ||
                    !player.getInventory().contains(ItemDefinition.SCRAP_ID,  data.getCost())) {
                player.getPacketSender().sendMessage("You do not have the required items!");
                return false;
            }
        }
    ...
    public void showUpgrade(Upgradeables upgrade) {
        ...
        player.getPacketSender().sendString(121018, "Scraps Req.: @whi@" + Misc.formatNumber(upgrade.getCost()));
        player.getPacketSender().sendString(121019, "Rate: @whi@" + upgrade.getSuccessRate() + "%");
    ...
    public void openInterface(Upgradeables.UpgradeType type) {
        ...
        player.getPacketSender().sendString(121018, "Scraps Req.: @whi@---");
        player.getPacketSender().sendString(121019, "Rate: @whi@---");
