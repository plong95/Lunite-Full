...
public class Herblore {
	...
	public static boolean cleanHerb(final Player player, final int herbId) {
		...
		if (player.getInventory().contains(herb.getGrimyHerb())) {
			...
			Achievements.doProgress(player, Achievements.Achievement.HERBLORE);
	...
	public static boolean finishPotion(final Player player, final int itemUsed, final int usedWith) {
		...
		player.setCurrentTask(new Task(2, player, false) {
			public void execute() {
				...
				AchievementsOLD.finishAchievement(player, AchievementDataOLD.MIX_A_POTION);
				if (!player.getInventory().contains(pot.getUnfinishedPotion())
						|| !player.getInventory().contains(pot.getItemNeeded())) {
					stop();
				}
	...
	enum SpecialPotion {
		...
		OVERLOAD(new Item[] { new Item(15309), new Item(15313), new Item(15317), new Item(15321), new Item(15325) },
				new Item(15333), 96, 300);
	...
	public static void handleSpecialPotion(Player p, int item1, int item2) {
		...
		for (Item INGREDIENTS : specialPotData.getINGREDIENTS()) {
			if (!p.getInventory().contains(INGREDIENTS.getId())
					|| p.getInventory().getAmount(INGREDIENTS.getId()) < INGREDIENTS.getAmount()) {
		...
		if (specialPotData == SpecialPotion.OVERLOAD) {
			AchievementsOLD.finishAchievement(p, AchievementDataOLD.MIX_AN_OVERLOAD_POTION);
			AchievementsOLD.doProgress(p, AchievementDataOLD.MIX_100_OVERLOAD_POTIONS);
		}
	}
}
