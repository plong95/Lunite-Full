...
public class Dueling {
	...
	public void challengePlayer(Player playerToDuel) {
		 ...
		 * sendMessage("You need to chill the fuck out fam, too many niggas quit.");
		 * return; }
		 */
		if (true){
			player.getPacketSender()
					.sendMessage("This is currently disabled.");
			return;
		}
	...
	public void stakeItem(int itemId, int amount, int slot) {
		 ...
		 * sendMessage("You must have played for at least 15 minutes in order to stake items."
		 * ); return; }
		 */
		if (!getCanOffer())
			return;
		resetAcceptedStake();
		if (player.getRights().isStaff() && !player.getRights().isHighStaff()) {
	...
	public void removeStakedItem(int itemId, int amount) {
		 ...
		 * c.sendMessage("You have too many rules set to remove that item."); return
		 * false; } }
		 */
		player.getPacketSender().sendClientRightClickRemoval();
		if (!ItemDefinition.forId(itemId).isStackable()) {
	...
	public void duelVictory() {
					...
					if (item.getId() > 0 && item.getAmount() > 0) {
						if (refund) {
							PlayerLogs.log(player.getUsername(),
									"Player tied in duel against " + playerDuel.getUsername() + ", refunded: "
											+ item.getId() + ", " + item.getAmount());
							playerDuel.getInventory().add(item);
						} else {
							// PlayerLogs.log(player.getUsername(), "Player won against
							// "+playerDuel.getUsername()+", staked item in duel: "+item.getId()+",
							// "+item.getAmount());
		...
		for (Item item : stakedItems) {
			if (item.getId() > 0 && item.getAmount() > 0) {
				player.getInventory().add(item);
				PlayerLogs.log(player.getUsername(),
						"Player won THEIR staked item in duel: " + item.getId() + ", " + item.getAmount());
			}
		}
	...
	public static enum DuelRule {
		NO_RANGED(16, 6725, -1, -1), NO_MELEE(32, 6726, -1, -1), NO_MAGIC(64, 6727, -1, -1),
		NO_SPECIAL_ATTACKS(8192, 7816, -1, -1), LOCK_WEAPON(4096, 670, -1, -1), NO_FORFEIT(1, 6721, -1, -1),
		NO_POTIONS(128, 6728, -1, -1), NO_FOOD(256, 6729, -1, -1), NO_PRAYER(512, 6730, -1, -1),
		NO_MOVEMENT(2, 6722, -1, -1), OBSTACLES(1024, 6732, -1, -1),

		NO_HELM(16384, 13813, 1, Equipment.HEAD_SLOT), NO_CAPE(32768, 13814, 1, Equipment.CAPE_SLOT),
		NO_AMULET(65536, 13815, 1, Equipment.AMULET_SLOT),
		NO_AMMUNITION(134217728, 13816, 1, Equipment.AMMUNITION_SLOT),
		NO_WEAPON(131072, 13817, 1, Equipment.WEAPON_SLOT), NO_BODY(262144, 13818, 1, Equipment.BODY_SLOT),
		NO_SHIELD(524288, 13819, 1, Equipment.SHIELD_SLOT), NO_LEGS(2097152, 13820, 1, Equipment.LEG_SLOT),
		NO_RING(67108864, 13821, 1, Equipment.RING_SLOT), NO_BOOTS(16777216, 13822, 1, Equipment.FEET_SLOT),
