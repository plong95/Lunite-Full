...
public class LoyaltyProgramme {
        ...
        IMMORTAL(-1, 43023, -22516) {
            ...
            boolean canBuy(Player p, boolean sendMessage) {
                ...
                if (!p.getUnlockedLoyaltyTitles()[4]) {
                    if (sendMessage)
                        p.getPacketSender()
                                .sendMessage("To unlock this title, you must kill 15 players without dying.");
                    return false;
                }
                return true;
            }
                        ...
                        if (p.getSkillManager().getMaxLevel(i) < 99) {
                            if (sendMessage)
                                p.getPacketSender().sendMessage(
                                        "You must be at least level 99 in every non-combat skill for this title.");
                            return false;
                        }
                    }
                }
                ...
                if (!p.getUnlockedLoyaltyTitles()[6]) {
                    for (int i = 0; i <= 6; i++) {
                        if (p.getSkillManager().getMaxLevel(i) < (i == 3 || i == 5 ? 990 : 99)) {
                            if (sendMessage)
                                p.getPacketSender().sendMessage(
                                        "You must be at least level 99 in every combat skill for this title.");
                            return false;
                        }
                    }
                }
            ...
            boolean canBuy(Player p, boolean sendMessage) {
                if (!p.getUnlockedLoyaltyTitles()[7]) {
                    if (!SkillManager.maxed(p)) {
                        if (sendMessage)
                            p.getPacketSender()
                                    .sendMessage("You must be at least level 99 in every skill for this title.");
                        return false;
                    }
                }
                unlock(p, this);
            ...
            boolean canBuy(Player p, boolean sendMessage) {
                if (!p.getUnlockedLoyaltyTitles()[9]) {
                    if (p.getPointsHandler().getLoyaltyPoints() < 100000) {
                        if (sendMessage)
                            p.getPacketSender().sendMessage(
                                    "To unlock this title, you must gain 100,000 Loyalty Points simultaneously.");
                        return false;
                    }
                }
                unlock(p, this);
            ...
            boolean canBuy(Player p, boolean sendMessage) {
                if (!p.getUnlockedLoyaltyTitles()[11]) {
                    if (p.getPointsHandler().getBarrowsChests() < 10) {
                        if (sendMessage)
                            p.getPacketSender()
                                    .sendMessage("To unlock this title, you must loot the barrows chest 10 times.")
                                    .sendMessage("You've currently looted the chest "
                                            + p.getPointsHandler().getBarrowsChests() + " time(s).");
                        return false;
                    }
                }
                unlock(p, this);
    ...
    public static void unlock(Player player, LoyaltyTitles title) {
        if (player.getUnlockedLoyaltyTitles()[title.ordinal()])
            return;
        AchievementsOLD.doProgress(player, AchievementDataOLD.UNLOCK_ALL_LOYALTY_TITLES);
    ...
    public static boolean handleButton(Player player, int button) {
        ...
        if (title != null) {
            if (title.canBuy(player, true)) {
                if (player.getTitle().equalsIgnoreCase(title.getName())) {
    ...
    public static void open(Player player) {
        ...
        player.getPacketSender()
                .sendString(43120, "Your Loyalty Points: " + player.getPointsHandler().getLoyaltyPoints())
                .sendInterface(43000);
    }
