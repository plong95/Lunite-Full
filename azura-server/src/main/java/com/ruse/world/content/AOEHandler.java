...
public class AOEHandler {
    ...
    public static void handleAttack(Character attacker, Character victim, int minimumDamage, int maximumDamage,
        ...
        if (radius == 0 || !Locations.Location.inMulti(victim) || victim.getLocation() == Locations.Location.WAVE_MINIGAME) {
        ...
        if (attacker.isPlayer() && victim.isPlayer()) {
            ...
            for (Iterator<? extends Character> $it = it; $it.hasNext(); ) {
                ...
                if (next.getPosition().isWithinDistance(victim.getPosition(), radius) && !next.equals(attacker)
                    ...
                    if (next.isNpc() && ((NPC)next).getId() == 28060) {
                        continue;
                    }
                    ...
                    switch (((Player) attacker).getLastCombatType()) {
                        case MELEE:
                            maxhit = Maxhits.melee(attacker, victim) / 3;
                            break;
                        case RANGED:
                            maxhit = Maxhits.ranged(attacker, victim) / 3;
                            break;
                        case MAGIC:
                            maxhit = Maxhits.magic(attacker, victim) / 3;
                            break;
                    }
                    Player player = (Player) attacker;

                    if (SpecialAmmo.isAmmo(player, SpecialAmmo.AOE)) {
                        maxhit *= SpecialAmmo.getAmmo(player).getBoost();
                    }

                    long calc = RandomUtility.inclusiveRandom(minimumDamage, maxhit);
