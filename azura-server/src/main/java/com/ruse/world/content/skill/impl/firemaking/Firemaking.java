		...
		player.setCurrentTask(new Task(addingToFire ? 2 : cycle, player, addingToFire ? true : false) {
			...
			public void execute() {
				...
				if (logData == Logdata.logData.OAK) {
					AchievementsOLD.finishAchievement(player, AchievementDataOLD.BURN_AN_OAK_LOG);
				} else if (logData == Logdata.logData.MAGIC) {
					AchievementsOLD.doProgress(player, AchievementDataOLD.BURN_100_MAGIC_LOGS);
					AchievementsOLD.doProgress(player, AchievementDataOLD.BURN_2500_MAGIC_LOGS);
				}
