...
public class ZulrahLogic implements CombatStrategy {
	...
	private static Animation shoot = new Animation(5069), charge = new Animation(5806), melee = new Animation(5807),
			dive = new Animation(5072), rise = new Animation(5073);

	private static Graphic toxic_cloud = new Graphic(310), fire = new Graphic(78), snakeling_summon = new Graphic(281);
	...
	private static void switchPhase(Character entity, Character victim) {
		...
		long currenthealth = zulrah.getConstitution();
			...
			TaskManager.submit(new Task(1, player, false) {
				...
				public void execute() {
					...
					if (tick == 5 && !zulrah.isRegistered()) {
						...
						NPC zulrah = new NPC(phase[Misc.randomMinusOne(phase.length)],
								new Position(move[rand].getX(), move[rand].getY(), player.getPosition().getZ()));
	...
	public boolean customContainerAttack(Character entity, Character victim) {
		...
		if (zulrah.getId() == phase[0]) { // do green phase
			if (Misc.getRandom(1) == 0) {
				TaskManager.submit(new Task(zulrah.getAttackSpeed(), zulrah, false) {
					...
					@Override
					public void execute() {
						if (entity == null || victim == null || zulrah.getConstitution() <= 0
								|| player.getConstitution() <= 0 || (zulrah.getLocation() != player.getLocation())) {
							stop();
							player.getMinigameAttributes().getZulrahAttributes().setRedFormDamage(0, false);
							zulrah.setChargingAttack(false);
							if (player.getRegionInstance() != null
						...
						if (Misc.isEven(tick)) {
							...
							zulrah.getCombatBuilder()
									.setContainer(new CombatContainer(zulrah, victim, 1, 1, CombatType.MAGIC, true));
							...
							zulrah.setChargingAttack(true);
							zulrah.getCombatBuilder()
									.setContainer(new CombatContainer(zulrah, victim, 1, 1, CombatType.RANGED, true));
							zulrah.performAnimation(shoot);
							new Projectile(zulrah, victim, 551, 44, 3, 43, 31, 0).sendProjectile(); // chaos elemental
																									// green
						}
						tick++;
					}
		...
		if (zulrah.getId() == phase[2]) { // do red phase
			// zulrah.forceChat("Current position: "+zulrah.getPosition().getX()+",
			// "+zulrah.getPosition().getY()+", "+zulrah.getPosition().getZ());
			TaskManager.submit(new Task(zulrah.getAttackSpeed(), zulrah, false) {
				int tick = 0;
				...
				public void execute() {
					...
					if (tick >= (ticksPerPhase)) {
						player.dealDoubleDamage(
								new Hit(player.getMinigameAttributes().getZulrahAttributes().getRedFormDamage() / 2,
										Hitmask.LIGHT_YELLOW, CombatIcon.DEFLECT),
								new Hit(player.getMinigameAttributes().getZulrahAttributes().getRedFormDamage() / 2,
										Hitmask.LIGHT_YELLOW, CombatIcon.DEFLECT));
						stop();
						player.getMinigameAttributes().getZulrahAttributes().setRedFormDamage(0, false);
