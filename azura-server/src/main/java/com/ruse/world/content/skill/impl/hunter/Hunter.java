...
public class Hunter {
	...
	public static boolean canLay(Player client) {
		if (!goodArea(client)) {
			client.getPacketSender().sendMessage("You need to be in a hunting area to lay a trap.");
			return false;
		}
		...
		for (final Trap trap : traps) {
			...
			if (trap.getGameObject().getPosition().getX() == client.getPosition().getX()
					&& trap.getGameObject().getPosition().getY() == client.getPosition().getY()) {
		...
		for (final NPC npc : HUNTER_NPC_LIST) {
			...
			if (x == npc.getPosition().getX() && y == npc.getPosition().getY()
					|| x == npc.getDefaultPosition().getX() && y == npc.getDefaultPosition().getY()) {
				client.getPacketSender()
		...
		if (client.getTrapsLaid() >= getMaximumTraps(client)) {
			client.getPacketSender()
					.sendMessage("You can only have a max of " + getMaximumTraps(client) + " traps setup at once.");
			return false;
		}
		return true;
	}
	...
	public static void handleRegionChange(Player client) {
		if (client.getTrapsLaid() > 0) {
			for (final Trap trap : traps) {
				if (trap == null)
					continue;
				if (trap.getOwner() != null && trap.getOwner().getUsername().equals(client.getUsername())
						&& !Locations.goodDistance(trap.getGameObject().getPosition(), client.getPosition(), 50)) {
					deregister(trap);
					client.getPacketSender()
							.sendMessage("You didn't watch over your trap well enough, it has collapsed.");
				}
			}
		}
	}
	...
	public static void dismantle(Player client, GameObject trap) {
		...
		if (theTrap != null && theTrap.getOwner() == client) {
			...
			client.getPacketSender().sendMessage("You cannot dismantle someone else's trap.");
	}
		...
		if (trap.getOwner() != null) {
			...
			if (trap instanceof SnareTrap)
				register(new SnareTrap(
						new GameObject(getObjectIDByNPCID(npc.getId()),
								new Position(trap.getGameObject().getPosition().getX(),
										trap.getGameObject().getPosition().getY())),
						TrapState.CAUGHT, 100, trap.getOwner()));
			else
				register(new BoxTrap(
						new GameObject(getObjectIDByNPCID(npc.getId()),
								new Position(trap.getGameObject().getPosition().getX(),
										trap.getGameObject().getPosition().getY())),
						TrapState.CAUGHT, 100, trap.getOwner()));
	...
	public static boolean hasLarupia(Player client) {
		return client.getEquipment().getItems()[Equipment.HEAD_SLOT].getId() == 10045
				&& client.getEquipment().getItems()[Equipment.BODY_SLOT].getId() == 10043
				&& client.getEquipment().getItems()[Equipment.LEG_SLOT].getId() == 10041;
	}
