...
/**
 * @author Gabriel Hannason Wrote this quickly!! Handles the RFD quest
 */
public class RecipeForDisaster {
	...
	public static void handleNPCDeath(final Player player, NPC n) {
		...
		player.getMinigameAttributes().getRecipeForDisasterAttributes().setWavesCompleted(
				player.getMinigameAttributes().getRecipeForDisasterAttributes().getWavesCompleted() + 1);
		switch (n.getId()) {
		case 3493:
		case 3494:
		case 3495:
		...
		if (player.getLocation() != Location.RECIPE_FOR_DISASTER
				|| player.getMinigameAttributes().getRecipeForDisasterAttributes().getWavesCompleted() == 6)
			return;
	...
	public static void openQuestLog(Player p) {
		...
		for (int i = 8147 + questIntro.length; i < 8147 + questIntro.length + questGuide.length; i++) {
			...
			if (questGuideIndex == 2) {
				if (p.getMinigameAttributes().getRecipeForDisasterAttributes().getWavesCompleted() > 0
						&& !p.getMinigameAttributes().getRecipeForDisasterAttributes().hasFinishedPart(questGuideIndex))
	...
	public static void openRFDShop(final Player player) {
		...
		Shop shop = new Shop(player, Shop.RECIPE_FOR_DISASTER_STORE, "Culinaromancer's chest", new Item(995),
				stockItems);
