...
public class DungeoneeringParty {
    ...
    public static void create(Player p) {
        ...
        if (p.getLocation() != Locations.Location.DUNGEONEERING && p.getLocation() != Locations.Location.CUSTOM_RAIDS_LOBBY) {
    ...
    public void teleportAndGiveKey() {
        ...
        for (Player member : getPlayers()) {
            if (member != null) {
                TeleportHandler.teleportPlayer(member, new Position(2078, 4421, member.getPosition().getZ()),
                        member.getSpellbook().getTeleportType());

                member.getInventory().add(13591, size > 1 ? 1 : 3); // enjoy.
