...
public class Gems {
	...
	enum GEM_DATA {
		...
		RED_TOPAZ(1629, 1613, 16, 25, new Animation(887), -1), SAPPHIRE(1623, 1607, 20, 50, new Animation(888), 1727),
		EMERALD(1621, 1605, 27, 68, new Animation(889), 1729), RUBY(1619, 1603, 34, 85, new Animation(892), 1725),
		DIAMOND(1617, 1601, 43, 108, new Animation(886), 1731),
		DRAGONSTONE(1631, 1615, 55, 138, new Animation(885), 1704), ONYX(6571, 6573, 67, 168, new Animation(885), 6585);
		...
		player.setCurrentTask(new Task(2, player, true) {
			...
			public void execute() {
				...
				if (data == GEM_DATA.DIAMOND) {
					AchievementsOLD.doProgress(player, AchievementDataOLD.CRAFT_1000_DIAMOND_GEMS);
				} else if (data == GEM_DATA.ONYX) {
					AchievementsOLD.finishAchievement(player, AchievementDataOLD.CUT_AN_ONYX_STONE);
				}
