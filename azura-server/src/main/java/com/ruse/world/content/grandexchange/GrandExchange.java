...
public class GrandExchange {
	...
	public static boolean handleButton(Player player, int id) {
		if (player.getInterfaceId() == MAIN_INTERFACE) {
			...
			case 24561:
				...
				if (player.getGrandExchangeSlots()[slot].getState() == GrandExchangeSlotState.PENDING_PURCHASE
						|| player.getGrandExchangeSlots()[slot].getState() == GrandExchangeSlotState.PENDING_SALE) {
					...
					if (offer.getAmountFinished() < offer.getAmount()) {
						if (offer.getType() == OfferType.BUYING) {
							offer.setCoinsCollect(
									offer.getTotalCost() - (offer.getAmountFinished() * offer.getPricePerItem()));
						} else {
			...
			case 24563:
				...
				if (player.getGrandExchangeSlots()[slot].getState() == GrandExchangeSlotState.PENDING_PURCHASE
						|| player.getGrandExchangeSlots()[slot].getState() == GrandExchangeSlotState.PENDING_SALE
						|| player.getGrandExchangeSlots()[slot].getState() == GrandExchangeSlotState.ABORTED
						|| player.getGrandExchangeSlots()[slot].getState() == GrandExchangeSlotState.FINISHED_PURCHASE
						|| player.getGrandExchangeSlots()[slot].getState() == GrandExchangeSlotState.FINISHED_SALE) {
			...
			if (player.getSelectedGeSlot() >= 0 && player.getGrandExchangeSlots()[player.getSelectedGeSlot()]
				...
				case 24726:
					if (!(player.getSelectedGeItem() > 0)) {
						...
						/*
						 * int amt = 0; for(Item t : player.getInventory().getValidItems()) {
						 * if(t.getId() == Item.getNoted(player.getSelectedGeItem()) || t.getId() ==
						 * Item.getUnNoted(player.getSelectedGeItem())) { amt += t.getAmount(); } }
						 * if(amt > Integer.MAX_VALUE) { amt = 0; }
						 */
				...
				case 24634:
				case 24734:
					setPricePerItem(player,
							((int) (player.getGePricePerItem() * 0.95)) == player.getGePricePerItem()
									? player.getGePricePerItem() - 1
									: (int) (player.getGePricePerItem() * 0.95));
					return true;
				case 24646:
				case 24746:
					setPricePerItem(player,
							((int) (player.getGePricePerItem() * 1.05)) == player.getGePricePerItem()
									? player.getGePricePerItem() + 1
									: (int) (player.getGePricePerItem() * 1.05));
					return true;
				case 24642:
				case 24742:
				...
				case 24738:
					...
					if (Item.sellable(player.getSelectedGeItem())) {
						...
						player.getPacketSender()
								.sendMessage("<img=5> <col=996633>This item does not have a base price set.");
					}
					return true;
				case 24602:
				case 24702:
				...
				case 24650:
				case 24750:
					if (player.getGrandExchangeSlots()[player.getSelectedGeSlot()].getOffer() == null
						...
						if (player.getGeQuantity() <= 0) {
							player.getPacketSender()
									.sendMessage("Please select a proper quantity before confirming your offer.");
							return true;
						}
						if (player.getGePricePerItem() <= 0) {
							player.getPacketSender()
									.sendMessage("Please select a proper price per item before confirming your offer.");
							return true;
						}
						...
						if (index == -1) {
							player.getPacketSender()
									.sendMessage("The Grand Exchange is currently too busy to accept your offer.");
							return true;
						}
						...
						if (offerType == OfferType.BUYING) {
							...
							if (!usePouch) {
								if (player.getInventory().getAmount(995) < cost) {
									player.getPacketSender()
											.sendMessage("You do not have enough coins to make this purchase.");
									return true;
								}
								player.getInventory().delete(995, cost);
							} else {
								player.getPacketSender().sendMessage("" + Misc.insertCommasToNumber("" + cost + "")
										+ " coins have been taken from your money pouch.");
							...
							if (player.getInventory().getAmount(player.getSelectedGeItem()) < player.getGeQuantity()) {
								player.getPacketSender()
										.sendMessage("You do not have that quantity of the item in your inventory.");
								return true;
							}
						...
						player.getGrandExchangeSlots()[player.getSelectedGeSlot()]
								.setOffer(new GrandExchangeOffer(item, player.getGeQuantity(), player.getUsername(),
										index, player.getGePricePerItem(), player.getSelectedGeSlot(), offerType));
						player.getGrandExchangeSlots()[player.getSelectedGeSlot()]
								.setState(id == 24650 ? GrandExchangeSlotState.PENDING_PURCHASE
										: GrandExchangeSlotState.PENDING_SALE);
			...
			case -10736:
			case -11736:
				if (player.getSelectedGeSlot() == -1
						|| player.getGrandExchangeSlots()[player.getSelectedGeSlot()].getOffer() == null
						|| !player.getClickDelay().elapsed(1000)) {
					return true;
				}
				if (player.getGrandExchangeSlots()[player.getSelectedGeSlot()]
						.getState() == GrandExchangeSlotState.PENDING_PURCHASE
						|| player.getGrandExchangeSlots()[player.getSelectedGeSlot()]
								.getState() == GrandExchangeSlotState.PENDING_SALE) {
					final GrandExchangeOffer offer = player.getGrandExchangeSlots()[player.getSelectedGeSlot()]
							.getOffer();
					if (offer.getAmountFinished() < offer.getAmount()) {
						if (offer.getType() == OfferType.BUYING) {
							offer.setCoinsCollect(
						...
						GrandExchangeOffers.setOffer(offer.getIndex(), null);
						player.getGrandExchangeSlots()[player.getSelectedGeSlot()]
								.setState(GrandExchangeSlotState.ABORTED);
						updateSlotStates(player);
						if (offer.getType() == OfferType.BUYING) {
	...
	public static void onLogin(Player player) {
		...
		for (GrandExchangeOffer o : offers) {
			...
			if (o2 == null || !(o.getId() == o2.getId() && o.getAmount() == o2.getAmount() && o.getBox() == o2.getBox()
					&& o.getType() == o2.getType() && o.getOwner().equals(o2.getOwner()))) {
				continue;
			}

			if (o2.getAmountFinished() != o.getAmountFinished() || o2.getCoinsCollect() != o.getCoinsCollect()
			...
			if (o2.getUpdateState() != null) {
				...
				if (o2.getFailAttempts() >= 3) {
					player.getPacketSender().sendMessage("")
							.sendMessage("<img=5> <col=996633>Perhaps you should try lowering the price on your "
		...
		if (player.getInterfaceId() == BUY_INTERFACE) {
			...
			player.getPacketSender()
					.sendMessage(cheapest == -1
							? "<img=5> <col=996633>Note: There are currently no players selling that item."
							: "<img=5> <col=996633>The cheapest offer found is currently selling at: "
			...
			player.getPacketSender()
					.sendMessage(mostExpensive == -1
							? "<img=5> <col=996633>Note: There are currently no players buying that item."
							: "<img=5> <col=996633>The most expensive offer found is currently buying at: "
	...
	public static void updateBuyInterface(Player player) {
		player.getPacketSender()
				...
				sendString(24672, "" + Misc.insertCommasToNumber("" + player.getGePricePerItem() + "") + " gp"). // Price
																													// per
																													// item
				sendString(24673,
						calculateTotalCost(player.getGePricePerItem(), player.getGeQuantity()) == -1 ? ("Too high!")
								: Misc.insertCommasToNumber(
										"" + calculateTotalCost(player.getGePricePerItem(), player.getGeQuantity()))
										+ " gp"); // Total amount
		if (player.getSelectedGeItem() <= 0) {
			player.getPacketSender().sendItemOnInterface(24680, -1, 1).sendString(24669, "Choose an item to exchange")
	...
	public static void updateViewPurchaseInterface(Player player, int boxIndex) {
		...
		player.getPacketSender().sendItemOnInterface(53780, geSlot.getOffer().getId(), 1)
				.sendItemOnInterface(53781, -1, 1).sendItemOnInterface(53782, -1, 1).sendString(53769, def.getName())
				.sendString(53770, def.getDescription())
				.sendString(53771, geSlot.getOffer().getAmountFinished() > 0
						? ("" + (geSlot.getOffer().getAmountFinished() >= geSlot.getOffer().getAmount() ? ("@gre@")
								: ("@yel@")) + ""
	...
	public static void updateSellInterface(Player player) {
		player.getPacketSender()
				...
				sendString(24772, "" + Misc.insertCommasToNumber("" + player.getGePricePerItem() + "") + " gp"). // Price
																													// per
																													// item
				sendString(24773,
						calculateTotalCost(player.getGePricePerItem(), player.getGeQuantity()) == -1 ? ("Too high!")
								: Misc.insertCommasToNumber(
										...
										+ " gp"); // Total amount
		if (player.getSelectedGeItem() <= 0) {
			player.getPacketSender().sendItemOnInterface(24780, -1, 1).sendString(24769, "Choose an item to exchange")
	...
	public static void updateViewSaleInterface(Player player, int boxIndex) {
		...
		player.getPacketSender().sendItemOnInterface(54780, item, 1).sendItemOnInterface(54781, -1, 1)
				.sendItemOnInterface(54782, -1, 1).sendString(54769, def.getName())
		...
		if (player.getGrandExchangeSlots()[boxIndex].getState() == GrandExchangeSlotState.PENDING_SALE
				|| player.getGrandExchangeSlots()[boxIndex].getState() == GrandExchangeSlotState.FINISHED_SALE
	...
	public static void collectItem(Player player, int id, int itemBox, OfferType type) {
		...
		case BUYING:
			if (player.getInventory().getFreeSlots() <= 0
					&& !(ItemDefinition.forId(returnId).isStackable() && player.getInventory().contains(returnId))) {
