...
import com.ruse.engine.task.impl.OwnerPotionTask;
...
public class Trading {
	...
	public void requestTrade(Player player2) {
		...
		if (player.getGameMode() == GameMode.IRONMAN
				&& !(player.getRights().isHighStaff() || player2.getRights().isHighStaff())) {
	...
	public void acceptTrade(int stage) {
		...
		int slotsAvailable = player.getInventory().freeSlots();
		for (Item item : player2.getTrading().offeredItems) {
			if (item == null || item.getId() == -1)
				continue;
			boolean inventoryContains = player.getInventory().contains(item.getId());
			boolean stackable = item.getDefinition().isStackable();
			if (inventoryContains && stackable)
				continue;
			else
				slotsAvailable --;
		}
		if (slotsAvailable < 0) {
			player.sendMessage("You do not have enough inventory space to accept this trade!");
			return;
		}
		if (stage == 2) {
	...
	public void giveItems() {
		...
		try {
			...
			if (ownerPot){
				...
				player.setPotionUsed(-1);
				player.setOverloadPotionTimer(0);
				player.getPacketSender().sendEffectTimer(0, 23374);
			}
