...
public class UltimateIronmanHandler {
	...
	public static boolean hasItemsStored(Player player) {
		if (player.getDungeoneeringIronEquipment().getValidItems().size()
				+ player.getDungeoneeringIronInventory().getValidItems().size() == 0) {
			return false;
		}
		return false;
	}
	...
	public static void handleQuickStore(Player player) {
		...
		if (player.getDungeoneeringIronEquipment().getValidItems().size() > 0
				|| player.getDungeoneeringIronInventory().getValidItems().size() > 0) {
		 ...
		 * player.getInventory().getValidItems().size() > 0) { player.getPacketSender().
		 * sendMessage("You cannot be wearing any gear, or have anything in your inventory."
		 * ); }
		 */
		...
		for (Item i : player.getInventory().getValidItems()) {
			...
			PlayerLogs.log(player.getUsername(),
					"Stored " + i.getAmount() + "x " + i.getDefinition().getName() + " in dung HCIM inv");
			player.getInventory().delete(i);
			// // System.out.println("Added "+i.getAmount()+"x "+i.getDefinition().getName() +"
			// to inventory list.");
		}
		...
		for (Item i : player.getEquipment().getValidItems()) {
			...
			// to equipment list.");
			PlayerLogs.log(player.getUsername(),
					"Stored " + i.getAmount() + "x " + i.getDefinition().getName() + " in dung HCIM equip");
			player.getEquipment().delete(i);
		}

		player.getPacketSender().sendMessage("<shad=0>@gre@Successfully stored " + equip + " equipment, and " + items
				+ (items > 1 ? " items" : " item") + ". Total: " + (equip + items) + "/39.");
		 ...
		 * // System.out.println("Found "+i.getAmount()+"x "+i.getDefinition().getName()
		 * +" in IronInventory"); }
		 * 
	...
	public static void handleQuickRetrieve(Player player) {
		...
		if (player.getEquipment().getValidItems().size() > 0
				&& player.getDungeoneeringIronEquipment().getValidItems().size() > 0) {
		...
		if (player.getInventory().getFreeSlots() < player.getDungeoneeringIronInventory().getValidItems().size()) {
			player.getPacketSender().sendMessage("You must have at least "
					+ player.getDungeoneeringIronInventory().getValidItems().size() + " free inventory slots first.");
			return;
		}

		int total = player.getDungeoneeringIronEquipment().getValidItems().size()
				+ player.getDungeoneeringIronInventory().getValidItems().size();
		if (total <= player.getInventory().getFreeSlots() && total > 0) {
			...
			for (Item i : player.getDungeoneeringIronEquipment().getValidItems()) {
				...
				PlayerLogs.log(player.getUsername(),
						"Retrieved " + i.getAmount() + "x " + i.getDefinition().getName() + " in dung HCIM equip");
				player.getDungeoneeringIronEquipment().delete(i);
			}
			for (Item i : player.getDungeoneeringIronInventory().getValidItems()) {
				player.getInventory().add(i);
				PlayerLogs.log(player.getUsername(),
						"Retrieved " + i.getAmount() + "x " + i.getDefinition().getName() + " in dung HCIM inv");
				player.getDungeoneeringIronInventory().delete(i);
			}
			player.getPacketSender().sendMessage("<shad=0>@gre@Your final "
					+ (equip + leftover > 1 ? equip + leftover + " items are " : "item is ") + "returned to you.");
			...
			if (player.getDungeoneeringIronInventory().getValidItems().size() > 0) {
				player.getPacketSender().sendMessage("<shad=0>@gre@You retrieve " + equip + " equipment, and have "
						+ (leftover > 1 ? leftover + " items" : "item") + " remaining.");
			} else {
			...
			player.getPacketSender().sendMessage(
					"<shad=0>@gre@Your final " + (inv > 1 ? inv + " items are " : "item is ") + "returned to you.");
