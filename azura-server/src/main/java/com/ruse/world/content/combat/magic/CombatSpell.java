...
public abstract class CombatSpell extends Spell {
	...
	public void startCast(Character cast, Character castOn) {
		...
		if (npc != null) {
			if (npc.getId() == 3496 || npc.getId() == 6278 || npc.getId() == 2000 || npc.getId() == 109
					|| npc.getId() == 3580 || npc.getId() == 2007 || npc.getId() == 9031) {
		...
		if (npc != null) {
			if (npc.getId() != 2000 && npc.getId() != 109 && npc.getId() != 3580 && npc.getId() != 2007&& npc.getId() != 9031) {
	 ...
	 * 
	 * @return the ID of the spell, or <tt>-1</tt> if there is no ID for this spell.
	 */
