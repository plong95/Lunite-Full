	...
	public static void handleBarCreation(int barId, Player player) {
		if (player.getOres()[0] > 0) {
			...
			if (barId != 2351) { // Iron bar - 50% successrate
				...
				if (barId == 2363) {
					AchievementsOLD.doProgress(player, AchievementDataOLD.SMELT_25_RUNE_BARS);
					AchievementsOLD.doProgress(player, AchievementDataOLD.SMELT_1000_RUNE_BARS);
				}
				Achievements.doProgress(player, Achievements.Achievement.SMITHING);
				...
				AchievementsOLD.finishAchievement(player, AchievementDataOLD.SMELT_AN_IRON_BAR);
				player.getInventory().add(barId, 1);
				player.getSkillManager().addExperience(Skill.SMITHING, getExperience(barId));
			} else
