...
public class Fishing {
	...
	public enum Spot {
		...
		BIGNET(313, new int[] { 353, 341, 363 }, 305, -1, new int[] { 16, 23, 46 }, false, new int[] { 20, 45, 100 },
				620),
		...
		// CAGE(312, new int[]{377}, 301, -1, new int[]{40}, false, new int[]{90}, 619),
		AFKNET(2067, new int[] { 10138 }, -1, -1, new int[] { 1 }, false, new int[] { 1 }, 618),
		ROCKTAIL(10091, new int[] { 15270 }, 309, 25, new int[] { 91 }, false, new int[] { 200 }, 380),
		FURY_SHARK(5748, new int[] { 17815 }, 311, -1, new int[] { 120 }, false, new int[] { 200 }, 618);
			...
			public void execute() {
				...
				if (cycle >= Misc.getRandom(1) + reqCycle) {
					...
					if (s.getRawFish()[fishIndex] == 331) {
						AchievementsOLD.finishAchievement(p, AchievementDataOLD.FISH_A_SALMON);

					} else if (s.getRawFish()[fishIndex] == 15270) {
						AchievementsOLD.doProgress(p, AchievementDataOLD.FISH_25_ROCKTAILS);
						AchievementsOLD.doProgress(p, AchievementDataOLD.FISH_2000_ROCKTAILS);
					} else if (s.getRawFish()[fishIndex] == 383) {
						Achievements.doProgress(p, Achievements.Achievement.DAILY_FISHING);
					}
					Achievements.doProgress(p, Achievements.Achievement.DAILY_FISHING);
					Achievements.doProgress(p, Achievements.Achievement.FISH_ALOT);
					Achievements.doProgress(p, Achievements.Achievement.FISHING);
