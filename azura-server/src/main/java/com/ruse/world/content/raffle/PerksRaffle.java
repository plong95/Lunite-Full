...
public class PerksRaffle {

    public static long entriesSize = 0;
    ...
    @Getter
    public static ArrayList<PerksRaffle> entriesList = new ArrayList<>();

    @Getter
    public static ArrayList<Item> REWARDS = new ArrayList<>();

    public static int MAXIMUM_ENTRIES = 250_000_000;
    ...
    public static void addEntry(Player player, int amount) {
        ...
        if (amount > 0) {
            player.sendMessage("<col=7F1425>[Server Perks]You have received " + Misc.insertCommasToNumber(amount) + " Raffle entries!");
        }
        entriesSize += amount;
        save();
    }
    ...
    public static void drawRaffle() {
        ...
        World.sendMessage(
                "<ico=854><col=4422FF><shad>Raffle </shad><col=7F1425><ico=854> [Perks]" +
                        " Weekly Raffle Winner: @red@" + winnerName+ "<col=7F1425> - Total Entries: @blu@" + Misc.formatNumber(entriesSize) );


        DiscordMessager.sendWebhook( "[" + UniversalRaffle.getDate() + "] Perks Raffle Winner: " + winnerName,
                Color.BLUE, "https://discordapp.com/api/webhooks/992012586801451170/1RcURJgeUDt8bdZUGmlxxwJlmEP6crp-bIFYiiwy-4tHKI4jwMeo8m1uHJ9WaHdZumCO");
    ...
    public static void claimReward(Player player) {
        player.sendMessage("@blu@You have won the Server Perks Weekly Raffle!");
    ...
    public static ArrayList<PerksRaffle> getEntriesList() {
        entriesList.clear();
        for (String name : ENTRIES.keySet()) {
            PerksRaffle drop = new PerksRaffle(name, ENTRIES.get(name));
    ...
    public PerksRaffle(String name, int entries) {
        this.name = name;
        this.entries += entries;
    }

    public static void save() {
        Path path = Paths.get("./data/saves/raffle/perks.json");
    ...
    public static void load() {
        ENTRIES.clear();
        try (FileReader fileReader = new FileReader("./data/saves/raffle/perks.json")) {
            ...
            if (reader.has("entries")) {
                PerksRaffle[] data = builder.fromJson(reader.get("entries"), PerksRaffle[].class);
                for (PerksRaffle price : data) {
