...
import com.ruse.util.infologs.ServerInfoLogs;
...
import com.ruse.world.content.raffle.UniversalRaffle;
...
public class World {
    ...
    public static void sendStaffMessage(String message) {
        players.stream().filter(p -> p != null && (p.getRights().isStaff()))
                .forEach(p -> p.getPacketSender().sendMessage(":staff:" + "{" + Misc.getCurrentServerTimeSeconds() +"}"+ message));
        DiscordMessager.sendStaffMessage(message);
        PlayerLogs.logStaff(message);
    }
        ...
        players.stream().filter(
                p -> p != null &&
                        (p.getRights() == PlayerRights.ADMINISTRATOR
                                || p.getRights() == PlayerRights.DEVELOPER
...
//                logger.severe("Error executing game-thread job. "+e.getMessage());
        ...
        if (System.currentTimeMillis() - lastTime > 50)
            System.out.println("First two CONTENT tickers took: " + (System.currentTimeMillis() - lastTime) + " ms");
        Bot.updatePlayers();
        Iron.initialize();
        if (System.currentTimeMillis() - lastTime > 50)
            System.out.println("Second two CONTENT tickers took: " + (System.currentTimeMillis() - lastTime) + " ms");
        Prime.initialize();
        GoldenOozau.initialize();
        Zamasu.initialize();
        //AprilFools.initialize();
        //VdayBoss.initialize();
        //VdayGifting.initialize();
        if (System.currentTimeMillis() - lastTime > 50)
            System.out.println("Third two CONTENT tickers took: " + (System.currentTimeMillis() - lastTime) + " ms");
        OnyxSpawn.initialize();
        ZenyteSpawn.initialize();
        TanzaniteSpawn.initialize();
        HydraSpawn.initialize();
        // BunnySpawn.initialize();
        //UncleSamSpawn.initialize();
      //  EggSpawns.initialize();
        CelestialZone.initialize();
        TriviaSystem.tick();
        LuniteGuardian.initialize();
        HantoWarrior.initialize();
        PartyChest.sequence();
        ...
        if (System.currentTimeMillis() - lastTime > 50)
            System.out.println("Fourth two CONTENT tickers took: " + (System.currentTimeMillis() - lastTime) + " ms");
        ServerPerks.getInstance().tick();
        CharacterBackup.sequence();
        if (System.currentTimeMillis() - lastTime > 50)
            System.out.println("Fifth two CONTENT tickers took: " + (System.currentTimeMillis() - lastTime) + " ms");
        ServerInfoLogs.sequence();
        if (System.currentTimeMillis() - lastTime > 50)
            System.out.println("Sixth two CONTENT tickers took: " + (System.currentTimeMillis() - lastTime) + " ms");
        ...
        if (System.currentTimeMillis() - lastTime > 25)
            System.out.println("Before 1 updating took: " + (System.currentTimeMillis() - lastTime) + " ms");
        lastTime = System.currentTimeMillis();

        players.forEach(playerUpdate::executeUpdate);

        if (System.currentTimeMillis() - lastTime > 25)
            System.out.println("Before 2 updating took: " + (System.currentTimeMillis() - lastTime) + " ms");
        lastTime = System.currentTimeMillis();

        synchronizer.arriveAndAwaitAdvance();

       // if (System.currentTimeMillis() - lastTime > 25)
        //    System.out.println("Before 3 updating took: " + (System.currentTimeMillis() - lastTime) + " ms");
        lastTime = System.currentTimeMillis();
        if (PRINT_TIMESTAMPS) {
            if (System.currentTimeMillis() - lastTime > 50)
